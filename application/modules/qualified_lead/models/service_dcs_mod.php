...
class Service_Dcs_mod extends CI_Model {
    ...
    {  
        filter_data('lead');
		$this->db->select('lead.*,lead.id as main_id,companies.name as companys_name,companies.industry as comp_industry,companies.*,cont.*,cont.name companies contact_person_name,lnd.note as lead_service_dcs_notes,ins.name as industry_name,con.country_name as country_name,st.state_name as state_name,ct.city_name as city_name,dept.name as department_name,desg.name as designation_name');
		...
		$this->db->join('lead_service_dcs_notes as lnd','lnd.lead_id =lead.id','left');
		...
		$this->db->from("leads_service_dcs as lead");
