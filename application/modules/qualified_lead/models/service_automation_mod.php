...
class Service_Automation_mod extends CI_Model {
   ...
   public $per_page  = "per_page";
    ...
    {  
        filter_data('lead');
		$this->db->select('lead.*,lead.id as main_id,companies.name as companys_name,companies.industry as comp_industry,companies.*,cont.*,cont.name as contact_person_name,lnd.note as lead_service_automation_notes,ins.name as industry_name,con.country_name as country_name,st.state_name as state_name,ct.city_name as city_name,dept.name as department_name,desg.name as designation_name');
		...
		$this->db->join('lead_service_automation_notes as lnd','lnd.lead_id =lead.id','left');
    ...
    $this->db->join('cities as ct','ct.id =companies.city_comp','left');
		...
		$this->db->from("leads_service_automation as lead");
	...
	public function is_unique_vendor_code($vendor_code) {
		...
		//pr($id);die;
        $query=$this->db->query("SELECT id FROM leads_service_automation WHERE vendor_code='$vendor_code' AND id!=".$id);
        // echo $this->db->last_query();exit;
        $count=$query->num_rows();		
        if ($count)
        {
