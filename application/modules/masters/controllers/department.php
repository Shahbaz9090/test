...
class Department extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->lang->load('department', get_site_language());
               
        $this->data['head']['title'] = "Department";
        $this->data['readonly'] = NULL;
        $this->data['base_url'] = base_url("masters/department");
        $this->export_limit = $this->config->item('export_limit');
        $this->delete_limit = $this->config->item('delete_limit');
        $this->table_name   = "department";

		$this->data['module'] = 'Masters/Department';
        $this->data['module_link'] = base_url("masters/department")."/list_items";
	...
	public function add()
	{
	   ...
	   $views[] = "department/form";

	   $this->data['submodule'] = 'Add Department';
	...
	{        
       $result = $this->department_mod->get($id);
       $this->data['result'] = $result;
       $this->data['readonly'] = 'readonly="true"';
       $this->data['action'] = "view";
       $views[] = "department/form_view"; 
		view_report($id);
	   $this->data['submodule'] = 'View Department';
       view_load($views,$this->data);
	}
	...
	{  	   
       if(isPostBack())
       {
           $r = $this->department_mod->update($id);
		   $this->data['error_msg'] ='';
            if($r==1){
                redirect($this->data['base_url']."/list_items/");
            }else if($r=='error'){
	   ...
	   $this->data['submodule'] = 'Edit Department';
	   view_load($views,$this->data);
	}
	...
	{  
        ...
        $this->data['grid']['cols'] = $this->department_mod->get_flexigrid_cols();
        ...
        $order_by='id';
        $order='desc';
        $result =  $this->department_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
        ...
        $this->data['submodule'] = 'Department List';
        view_load($views, $this->data);
	}
	...
	{ 
		...
		$perPage = $this->department_mod->perPage($user->id);
		//pr($perPage);die;
        if($perPage) {
        } else {
            ...
            $pageArr = array(
                ...
                'user_id' => $user->id);
            $this->department_mod->insert_perPage($pageArr);
        }
        ...
        $data = $this->department_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
       ...
       //pr($data);die;
        ...
        $data['grid']['cols'] = $this->department_mod->get_flexigrid_cols();
    ...
    public function export()
    {
       ...
       $data = $this->department_mod->export();

       export_report($items_data);
       array_to_csv($data,"Department.csv");
    }
  ...
  // ------------------------------------------------------------------------
    ...
    public function status($id = null) {
        $result = $this->department_mod->get($id);
        $r = $this->department_mod->status_update($id, $result->status);
		//pr($r);die;
        if($r) {
            redirect($this->data['base_url'] . "/list_items");
        }
