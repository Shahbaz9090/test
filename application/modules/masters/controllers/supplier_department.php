...
class Supplier_Department extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('supplier_department_mod');
        $this->lang->load('supplier_department', get_site_language());
               
        $this->data['head']['title'] = "Department";
        $this->data['readonly'] = NULL;
        $this->data['base_url'] = base_url("masters/supplier_department");
        $this->export_limit = $this->config->item('export_limit');
        $this->delete_limit = $this->config->item('delete_limit');
        $this->table_name   = "supplier_department";

		$this->data['module'] = 'Masters/Supplier_Department';
        $this->data['module_link'] = base_url("masters/supplier_department")."/list_items";
	...
	public function add()
	{
	   if(isPostBack())
       {
            $id = $this->supplier_department_mod->add();
            if($id){
	   ...
	   $views[] = "supplier_department/form";

	   $this->data['submodule'] = 'Add Supplier-Department';
	...
	{        
       $result = $this->supplier_department_mod->get($id);
       $this->data['result'] = $result;
       $this->data['readonly'] = 'readonly="true"';
       $this->data['action'] = "view";
       $views[] = "supplier_department/form_view"; 
		view_report($id);
	   $this->data['submodule'] = 'View Supplier-Department';
       view_load($views,$this->data);
	}
	...
	{  	   
       if(isPostBack())
       {
           $r = $this->supplier_department_mod->update($id);
            if($r==1){
                redirect($this->data['base_url']."/list_items/");
            }else if($r=='error'){
	   ...
	   $this->data['submodule'] = 'Edit Supplier-Department';
	   view_load($views,$this->data);
	}
	...
	{  
		$views[] = "supplier_department/list";
        $this->data['title'] = lang('list_title');
        $this->data['place_holder'] = "Enter filter terms here";        
        $this->data['action'] = "list";

        $this->data['grid']['cols'] = $this->supplier_department_mod->get_flexigrid_cols();
        ...
        $order_by='id';
        $order='desc';
        $result =  $this->supplier_department_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
        ...
        $this->data['submodule'] = 'Supplier-Department List';
        view_load($views, $this->data);
	}
	...
	{ 
		...
		$perPage = $this->supplier_department_mod->perPage($user->id);
		//pr($perPage);die;
        if($perPage) {
        } else {
            ...
            $pageArr = array(
                ...
                'user_id' => $user->id);
            $this->supplier_department_mod->insert_perPage($pageArr);
        }
        ...
        $data = $this->supplier_department_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
       ...
       //pr($data);die;
        ...
        $data['grid']['cols'] = $this->supplier_department_mod->get_flexigrid_cols();
    ...
    public function export()
    {
       ...
       $data = $this->supplier_department_mod->export();

       export_report($items_data);
       array_to_csv($data,"Supplier-Department.csv");
    }
  ...
  // ------------------------------------------------------------------------
    ...
    public function status($id = null) {
        $result = $this->supplier_department_mod->get($id);
        $r = $this->supplier_department_mod->status_update($id, $result->status);
		//pr($r);die;
        if($r) {
            redirect($this->data['base_url'] . "/list_items");
        }
