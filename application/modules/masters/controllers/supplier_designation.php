...
class Supplier_Designation extends CI_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('supplier_designation_mod');
        $this->lang->load('supplier_designation', get_site_language());
               
        $this->data['head']['title'] = "Supplier-Designation";
        $this->data['readonly'] = NULL;
        $this->data['base_url'] = base_url("masters/supplier_designation");
        $this->export_limit = $this->config->item('export_limit');
        $this->delete_limit = $this->config->item('delete_limit');
        $this->table_name   = "supplier_designation";

		$this->data['module'] = 'Masters/Supplier-Designation';
        $this->data['module_link'] = base_url("masters/supplier_designation")."/list_items";
	...
	public function add()
	{
	   if(isPostBack())
       {
            $id = $this->supplier_designation_mod->add();
            if($id){
	   ...
	   $views[] = "supplier_designation/form";

	   $this->data['submodule'] = 'Add Supplier-Designation';
	...
	{        
       $result = $this->supplier_designation_mod->get($id);
       $this->data['result'] = $result;
       $this->data['readonly'] = 'readonly="true"';
       $this->data['action'] = "view";
       $views[] = "supplier_designation/form_view"; 
		view_report($id);
	   $this->data['submodule'] = 'View Supplier-Designation';
       view_load($views,$this->data);
	}
	...
	{  	   
       if(isPostBack())
       {
           $r = $this->supplier_designation_mod->update($id);
            if($r==1){
                redirect($this->data['base_url']."/list_items/");
            }else if($r=='error'){
	   ...
	   $this->data['submodule'] = 'Edit Supplier-Designation';
	   view_load($views,$this->data);
	}
	...
	{  
		$views[] = "supplier_designation/list";
		
        $this->data['title'] = lang('list_title');
        $this->data['place_holder'] = "Enter filter terms here";        
        $this->data['action'] = "list";

        $this->data['grid']['cols'] = $this->supplier_designation_mod->get_flexigrid_cols();
        ...
        $order_by='id';
        $order='desc';
        $result =  $this->supplier_designation_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
        ...
        $this->data['submodule'] = 'Supplier-Designation List';
        view_load($views, $this->data);
	}
	...
	{ 
		...
		$perPage = $this->supplier_designation_mod->perPage($user->id);
		//pr($perPage);die;
        if($perPage) {
        } else {
            ...
            $pageArr = array(
                ...
                'user_id' => $user->id);
            $this->supplier_designation_mod->insert_perPage($pageArr);
        }
        ...
        $data = $this->supplier_designation_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
       ...
       //pr($data);die;
        ...
        $data['grid']['cols'] = $this->supplier_designation_mod->get_flexigrid_cols();
  ...
  // ------------------------------------------------------------------------
    ...
    public function status($id = null) {
        $result = $this->supplier_designation_mod->get($id);
        $r = $this->supplier_designation_mod->status_update($id, $result->status);
		//pr($r);die;
        if($r) {
            redirect($this->data['base_url'] . "/list_items");
        }
