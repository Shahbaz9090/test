...
class Industry_mod extends CI_Model {
    ...
    function add()
    {
        ...
        $this->form_validation->set_rules('name', lang('name'), 'trim|required|is_unique[industry.name]');
    ...
    public function is_unique_names($val) {
		...
		//pr($id);die;
        $query=$this->db->query("SELECT id FROM industry WHERE name='$val' AND id!=".$id);
        // echo $this->db->last_query();exit;
