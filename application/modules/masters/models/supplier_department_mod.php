...
class Supplier_Department_mod extends CI_Model {

    public $table     = "supplier_department";
    ...
    function add()
    {
        ...
        $this->form_validation->set_rules('name', lang('name'), 'trim|required|is_unique[supplier_department.name]');
    ...
    public function is_unique_names($val) {
		...
		//pr($id);die;
        $query=$this->db->query("SELECT id FROM supplier_department WHERE name='$val' AND id!=".$id);
        // echo $this->db->last_query();exit;
