...
class Supplier_vendor_mod extends CI_Model {

    public $table     = "supplier_vendor";
    ...
    function add()
    {
        ...
        $st = $this->is_unique_names(strtolower($this->input->post('name',TRUE)),$this->input->post('country',TRUE));
    ...
    public function is_unique_names($val,$country) {
		//$database=DEFAULT_DATABASE;
        // echo "SELECT id FROM $this->table WHERE country_name='$name' AND id!=".$country_id;die;
        $id = $this->uri->segment(4);
		if(empty($id)){
			$query=$this->db->query("SELECT id FROM supplier_vendor WHERE name='$val' AND country='$country'");
		}else{
			$query=$this->db->query("SELECT id FROM supplier_vendor WHERE name='$val' AND country='$country' AND id!=".$id);
        }
		 //echo $this->db->last_query();exit;
