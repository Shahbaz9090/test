...
class Supplier_Designation_mod extends CI_Model {

    public $table     = "supplier_designation";
    ...
    function add()
    {
        ...
        $this->form_validation->set_rules('name', lang('name'), 'trim|required|is_unique[supplier_designation.name]');
    ...
    function ajax_list_items($text, $limit, $offset, $order_by='id', $order='desc') {
        ...
        if($text) {
            $this->db->like("CONCAT($this->table.name,$this->table .short_code)",
                $text);
        }
    ...
    public function is_unique_names($val) {
		...
		//pr($id);die;
        $query=$this->db->query("SELECT id FROM supplier_designation WHERE name='$val' AND id!=".$id);
        // echo $this->db->last_query();exit;
	...
	function export(){
		...
		$this->db->select("$this->table .id,$this->table .name,$this->table .short_code");
        $this->db->from("$this->table");
