	  ...
	  if($result){	
       ...
       if($end_date!=null){
		...
		foreach($result as $key=>$value){
				$index=$key;
				$index++;
                $assign_user_id=isset($value->assign_user_id) ? $value->assign_user_id : null;
				$pipeline=get_final_joborder_pipeline($value->id,$start_date,$end_date);
				$submittal=get_final_joborder_submittal($value->id,$start_date,$end_date,$assign_user_id,$report_type);
                //$check_serviced = $pipeline+$submittal;
                $serviced = get_final_joborder_seriviced_job($value->id,$start_date,$end_date,$assign_user_id,$report_type);
		...
		<tr class="msg_box" id="<?=$index?>">
          <td width='5%'> <?=$index?></td>
			<?php
				...
				if(in_array("submitted",$fields)){
					//echo "<td width='3%'> <a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/5' target='_new'>".get_final_joborder_submittal($value->id,$start_date,$end_date,$value->assign_user_id)."</a></td>"; //===comment by sumit on 3 oct 2016 and open down line====//
					echo "<td width='3%'> <a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/5' target='_new'>".get_final_joborder_submittal($value->id,$start_date,$end_date,$assign_user_id)."</a></td>";
				}
