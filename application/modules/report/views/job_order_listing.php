...
<div class="span2 del" style="float:right;">
...
<div class="tableBg" style="float: left;">
<table width="100%">
	<tr>
	   <?php
	    $perm= _check_perm();
        ...
        echo '<th class="th" width="5%">Job Id</th>';
 	     ...
 	     if($fields){
              ...
              if(in_array("sales_reject",$fields)){				
				echo "<th class='th' width='3%'><a href='javascript:void(0);' class='tip' style='text-decoration:none;color:#fff;' title='Candidate Sales Reject'>S R</a></th>";
			  }
              if(in_array("sceen_reject",$fields)){				
				echo "<th class='th' width='3%'><a href='javascript:void(0);' class='tip' style='text-decoration:none;color:#fff;' title='Candidate Screen Reject'>S R</a></th>";
			  }
	  ...
	  if($result){	
       ...
       if($end_date!=null){
		...
		foreach($result as $key=>$value){
				$start_date =date("Y-m-d",strtotime($value->modified_time));
				$end_date =date("Y-m-d",strtotime($value->modified_time));
				$index=$key;
				$index++;
				$get_final_joborder_pipeline_new = get_final_joborder_pipeline_new($value->id,$start_date,$end_date,$user_list,$user_type,$report_type,$value->assign_user_id);
				$pipeline=$get_final_joborder_pipeline_new;
				$get_final_joborder_submittal_new = get_final_joborder_submittal_new($value->id,$start_date,$end_date,$user_list,$user_type,$report_type,$value->assign_user_id);
				$submittal=$get_final_joborder_submittal_new;
                $serviced = get_final_joborder_seriviced_job_new($value->id,$start_date,$end_date,$user_list,$user_type,$report_type);
                $current_serviced = $serviced;
				$can_jon_get_final_joborder_data_new=get_final_joborder_data_new('11',$value->id,$start_date,$end_date,$user_list,$user_type,$report_type);
				$candidate_join=$can_jon_get_final_joborder_data_new;
				$candidate_screen_reject_get_final_joborder_data_new = get_final_joborder_data_new('10',$value->id,$start_date,$end_date,$user_list,$user_type,$report_type);
                $candidate_screen_reject=$candidate_screen_reject_get_final_joborder_data_new;
				$candidate_interview_get_final_joborder_data_new = get_final_joborder_data_new('6',$value->id,$start_date,$end_date,$user_list,$user_type,$report_type);
                $candidate_interview=$candidate_interview_get_final_joborder_data_new;
				$candidate_interview_reject_get_final_joborder_data_new = get_final_joborder_data_new('3',$value->id,$start_date,$end_date,$user_list,$user_type,$report_type);
                $candidate_interview_reject=$candidate_interview_reject_get_final_joborder_data_new;
                $candidate_no_contact_get_final_joborder_data_new =get_final_joborder_data_new('1',$value->id,$start_date,$end_date,$user_list,$user_type,$report_type);
				$candidate_no_contact=$candidate_no_contact_get_final_joborder_data_new;
                $candidate_offered_get_final_joborder_data_new = get_final_joborder_data_new('7',$value->id,$start_date,$end_date,$user_list,$user_type,$report_type);
				$candidate_offered=$candidate_offered_get_final_joborder_data_new;
                $candidate_offered_decline_get_final_joborder_data_new = get_final_joborder_data_new('9',$value->id,$start_date,$end_date,$user_list,$user_type,$report_type);
				$candidate_offered_decline=$candidate_offered_decline_get_final_joborder_data_new;
                $candidate_sales_reject_get_final_joborder_data_new = get_final_joborder_data_new('2',$value->id,$start_date,$end_date,$user_list,$user_type,$report_type);
				$candidate_sales_reject=$candidate_sales_reject_get_final_joborder_data_new;
	...
	?>
		<tr class="msg_box" id="<?=$index?>">
          <td width='5%'> <?=$index?></td>
			<?php
					$job_lst_url=base_url().'job_order/view';
			        echo "<td width='5%'> <a href='$job_lst_url/$value->id' target='_blank'>$value->id($value->reactive)</a></td>";
                ...
                if(in_array("recruiter",$fields)){
					//echo "<td width='10%'> $value->recruiter </td>";
					//echo "<td width='10%'> ".get_recruiter_data($value->id,$start_date,$end_date,$rep_Recruiter)."</td>";
					echo "<td width='10%'> ".get_recruiter_data_new($value->id,$start_date,$end_date,$value->assign_user_id)."</td>";
				}
			...
			if(in_array("serviced",$fields)){					
				...
				if(in_array("pipeline",$fields)){					
					echo "<td width='3%'><a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/0' target='_new'> ".$get_final_joborder_pipeline_new."</a></td>";
				}

				if(in_array("submitted",$fields)){
					echo "<td width='3%'><a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/5' target='_new'> ".$get_final_joborder_submittal_new."</a></td>";
				}
                ...
                if(in_array("sales_reject",$fields)){
					echo "<td width='3%'> <a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/11' target='_new'>".$candidate_sales_reject_get_final_joborder_data_new."</a></td>";
				}
                if(in_array("sceen_reject",$fields)){
					echo "<td width='3%'> <a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/10' target='_new'>".$candidate_screen_reject_get_final_joborder_data_new."</a></td>";
				}
                if(in_array("interview",$fields)){
					echo "<td width='3%'> <a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/6' target='_new'>".$candidate_interview_get_final_joborder_data_new."</a></td>";
				}
                if(in_array("interview_reject",$fields)){
					echo "<td width='3%'> <a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/3' target='_new'> ".$candidate_interview_reject_get_final_joborder_data_new." </a></td>";
				}
                if(in_array("no_contact",$fields)){
					echo "<td width='4%'> <a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/1  ' target='_new'>".$candidate_no_contact_get_final_joborder_data_new."</a></td>";
				}
                if(in_array("offered",$fields)){
					echo "<td width='3%'> <a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/7' target='_new'>".$candidate_offered_get_final_joborder_data_new."</a></td>";
				}
                if(in_array("offered_decline",$fields)){
					echo "<td width='3%'> <a href='".SITE_PATH."report/job_order/view/$value->id/$value->company_id/9' target='_new'>".$candidate_offered_decline_get_final_joborder_data_new."</a></td>";
				}
                if(in_array("candidate_join",$fields)){
					echo "<td width='3%'> ".$can_jon_get_final_joborder_data_new."</td>";
				}
	...
	<tr> <td style="width: 5%;font-weight: bold;text-align: left;">Total</td>
              ...
              <?php
               ...
               if(in_array("pipeline",$fields)){ ?>		
                ...
                if(in_array("interview",$fields)){ ?>
				<td  style="width='3%';font-weight: bold;"><span id="total_interview_span"><?//=get_job_order_interview(6,$start_date,$end_date,$user_list)?><?=$total_interview?></span></td>
                <?php
				}
                if(in_array("interview_reject",$fields)){ ?>
