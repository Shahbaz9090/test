...
/**
 * CodeIgniter Sales_spares_enquiry_china Controller
 *
 * @package		CodeIgniter
...
class Sales_spares_enquiry_china extends MY_Controller {
    ...
    public function __construct()
    {
        ...
        $this->data['title'] 	        = "Sales Spares Enquiry";
        $this->data['title_for_view']   = "Sales Spares";
        $this->data['sales_order_url']  = base_url('order/sales_spares_order');
        $this->data['readonly'] 		= NULL;
        $this->data['base_url'] 		= base_url("enquiry/sales_spares_enquiry_china/");
		$this->data['module'] 			= 'Enquiry';
        $this->data['module_url'] 		= base_url("enquiry/sales_spares_enquiry_china/");
        // $this->table_name   			= "enquiry";
		
        $this->data['table_name']       = "enquiry";
        $this->load->library('enquiry_china_lib', [$this, 'Sales Spares List', 1] );
    ...
    {   
        ...
        $this->enquiry_china_lib->delete_quotation($quotation_id, $enquiry_id);
    }
  	...
  	public function add_more_email($enquiry)
    {
        ...
        $this->enquiry_china_lib->check_custom_permission($enquiry,$this->data['table_name']);
        $this->enquiry_china_lib->add_more_email($enquiry,1);//second parameter is for enquiry type
    }
    
  	public function doc_download($id = NULL, $recID)
    {
        if(!isset($id) || empty($id) || !isset($recID) || empty($recID))
        {
            redirect($this->data['module_url'].'/list_items');
        }
        $this->enquiry_china_lib->check_custom_permission($recID,$this->data['table_name']);
    ...
    public function view_email($email_id,$type)
    {
        if($type == 1)
        {
            redirect(base_url().'mail/sales_spares_email/view/'.$email_id);
        }
        else
        {
            redirect(base_url().'mail_china/sales_spares_email_china/view/'.$email_id);
        }
    }
    ...
    public function export()
     {
        ...
        $is_china_user = true;
         ...
         $query = $this->db->get();
         $data = $query->row();
         //pr($data->country==2);die;
         if($data->country==2){
            $table_columns = ["enq_no"=>"Enq. No.","internal_subject"=>"Internal Subject", "from_client"=>"From Client", "inch_name"=>"Inch Engineer", "cfit_name"=>"CFIT Engineer", "status_name"=>"India Status", "china_status_name"=>"China Status", "to_cfit"=>"Inch", "from_cfit"=>"CFIT", "to_client"=>"To Client"];
         }else{
        ...
        //      if($val->is_china_sent=='1'){
        //         pr("yess"); 
        //      }else{
        //         pr("no");
        //      }
        // } die;
        $filename = "Sales Spares enquiry China-" . date('dmYhis'). ".xls";
        //pr($table_columns);
        //pr($filename);
        //pr($result);die;
