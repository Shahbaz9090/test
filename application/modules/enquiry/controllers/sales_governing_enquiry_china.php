...
class Sales_governing_enquiry_china extends MY_Controller {
    ...
    public function __construct()
    {
        ...
        $this->data['title'] 	        = "Sales Governing Enquiry";
        $this->data['title_for_view']   = "Sales Governing";
        $this->data['sales_order_url']  = base_url('order/sales_governing_order');
        $this->data['readonly'] 		= NULL;
        $this->data['base_url'] 		= base_url("enquiry/sales_governing_enquiry_china/");
        $this->data['module_url'] 		= base_url("enquiry/sales_governing_enquiry_china/");
        $this->data['table_name']       = "enquiry";
		$this->data['module'] 			= 'Enquiry';
        $this->load->library('enquiry_china_lib', [$this, 'Sales Governing List', 2] );
    }
    ...
    public function add_more_email($enquiry)
    {
        ...
        $this->enquiry_china_lib->add_more_email($enquiry,2);//second parameter is for enquiry type
    }
    ...
    public function view_email($email_id,$type)
    {
        if($type == 1)
        {
            redirect(base_url().'mail/sales_governing_email/view/'.$email_id);
        }
        else
        {
            redirect(base_url().'mail_china/sales_governing_email_china/view/'.$email_id);
        }
    }
    ...
    public function export()
     {
        ...
        $filename = "Sales Governing enquiry China-" . date('dmYhis'). ".xls";
        //pr($table_columns);
        //pr($filename);
        //pr($result);die;
