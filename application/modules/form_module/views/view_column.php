...
<div class="modelLG modal fade" id="sort_column" tabindex="-1" role="dialog" aria-labelledby="myModalLabel2" aria-hidden="true">
  	<div class="modal-dialog modal-lg">
		<div class="modal-content">
		  	<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only"></span></button>
...
<script type="text/javascript">
	function remove_block(block_index)
	{
		if(confirm("Are you sure to delete this block.\n All Filed of this block will be lost permanently"))
		{
			...
			{
				$.ajax({
					url:'<?php echo base_url(); ?>form_module/remove_block/',
					type:"POST",
					dataType:'json',
					data: token_name+"="+token_hash+"&block_index="+block_index+'&frm_id='+frm_id,
	...
	$( function() {
		...
		$('.block_rename').on('keyup',function(e) {
		    ...
		    if(keycode == '13') {
		        ...
		        {
		        	$.ajax({
						url:'<?php echo base_url(); ?>form_module/rename_block/',
						type:"POST",
						dataType:'json',
						data: token_name+"="+token_hash+"&block_name="+block_name+"&block_index="+block_index+'&frm_id='+frm_id,
						success:function(res)
						{
							// console.log(res.status);
							if(res.status==1)
		...
		$("#save-block-btn").click(function(){
			...
			{
				$.ajax({
					url:'<?php echo base_url(); ?>form_module/add_block/',
					type:"POST",
					dataType:'json',
					data: token_name+"="+token_hash+"&block_name="+block_name+'&frm_id='+frm_id,
					success:function(res)
					{
						// console.log(res.status);
						if(res.status==1)
		...
		$(document).on('click','#moveColumnBtn',function(){
	        ...
	        $.ajax({
	        	url:'<?php echo base_url(); ?>form_module/reorder_column/',
				type:"POST",
				cache: false,
				dataType:'json',
				data: token_name+"="+token_hash+'&frm_id='+frm_id+"&reorder_data="+dataString,
				success:function(res)
				{
					if(res.status==1)
					{
		...
		$(document).on('click','#moveBlockBtn',function(){
	        ...
	        $.ajax({
	        	url:'<?php echo base_url(); ?>form_module/reorder_block/',
				type:"POST",
				cache: false,
				dataType:'json',
				data: token_name+"="+token_hash+'&frm_id='+frm_id+"&reorder_block="+dataString,
				success:function(res)
				{
					if(res.status==1)
					{
