...

{
...

    {
        ...
        $this->data["new"] = $this->get_module(); 

        $this->data["on_edit"] = $this->get_module_for_edit(); 
        ...
        if($uri=='dynamic_add'){
            ...
            $this->data['module'] = $this->data['new']->form_label;

            $this->data['module_link'] = base_url("form_module/dynamic")."/".$this->data['new']->form_name;

        }else{

            $this->data['module'] = $this->data['on_edit']->form_label;

            $this->data['module_link'] = base_url("form_module/dynamic")."/".$this->data['on_edit']->form_name;
                ...
                foreach ($reorder_module as $rbkey => $module) {
...

                    {
                        ...
                        foreach ($module->children as $key2 => $children) {
                            ...
                            $this->db->update("inch_form",['order_by'=>$key2+1,'parent_id'=>$module->id]);
   ...
   /* public function ajax_list_items()
    ...
    {   
        ...
        //$perPage = $this->form_module_mod->perPage($user->id);
        ...
        /*if($perPage) {
            ...
            $this->form_module_mod->insert_perPage($pageArr);
        ...
        $data = $this->form_module_mod->ajax_list_items($text,$limit,$offset,$order_by, $order);
        ...
        // var_dump($this->input->post('limit'););die;
        ...
        $result = $this->form_module_mod->ajax_list_items($text, $limit, $offset, $order_by, $order, $table_name);
        ...
        $data['grid']['result'] = $result['result'];

        $data['grid']['total'] = $result['total'];

        $data['grid']["page_offset"] = $offset;

        $data['grid']["limit"] = $limit;

        $data['grid']["order_by"] = $order_by;

        $data['dynamic_module_name'] = $table_name;
...

    {
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($this->db->field_exists(strtolower($requ['field_name']), 'inch_' . $result->form_name)) {
...

                {
                    ...
                    if ($res) {
                        ...
                        $sql = "ALTER TABLE inch_" . $result->form_name .$alter.$column_df_value;
...

    {
        ...
        $columns    = $this->db->field_data('inch_'.$form_name);
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (strtolower($requ['previous_name']) != strtolower($requ['field_name']) && $this->db->field_exists(strtolower($requ['field_name']), 'inch_' . $result->form_name)) {
                    ...
                    if (isset($result->form_data) && !empty($result->form_data)) {
                        ...
                        if ($res) {
                            ...
                            $alter = "ALTER TABLE inch_" . $result->form_name . " CHANGE " . $col->name . " " . strtolower($requ['field_name'])." ".$alter.$data_type.$column_df_value;
...

    {
        ...
        $qry = $this->db->update('inch_form', $update);

        //$qry = $this->db->delete('inch_form');
        ...
        if ($qry) {
            ...
            //$this->db->query("DROP TABLE inch_".$result->form_name);
...

    {
        ...
        if (isPostBack()) {
            ...
            if ($this->db->field_exists($requ['field_name'], 'inch_' . $result->form_name)) {
                ...
                if ($res) {
                    ...
                    $sql = "ALTER TABLE inch_" . $result->form_name . " $alter";
...

    {
        ...
        if (isPostBack()) {
            ...
            if ($result) {
                ...
                $res = $this->db->insert('inch_' . $result->form_name, $postData);
...

    {
        ...
        if (isPostBack()) {
            ...
            if ($result) {
                ...
                $res = $this->db->update('inch_form', $update);
        ...
        if (isPostBack()) {
            ...
            if ($result) {
                ...
                if ($col) {
                    ...
                    if ($this->db->field_exists($col->name, "inch_$result->form_name")) {

                        $this->db->query("ALTER TABLE inch_$result->form_name DROP $col->name");
...

    {
...

        {
            ...
            $result   = $this->db->select('controller,method')->get_where('inch_bypass_permission',['status'=>1])->result_array();

            // pr($result);die;

            cache_opration('set','bypass_permission', $result);
        ...
        $result   = $this->form_module_mod->dynamic_list_items($id, $offset, $limit);
        ...
        $data["new"] = $this->get_module();

        $views[]       = 'dynamic_form_list';

        $data['title'] = $data['new']->form_label." List Items";
...

    {
        ...
        if (isPostBack()) {
            ...
            $this->db->insert('inch_' . $res->form_name, $postData);

            if($res->form_name=='bypass_permission')

            {

                cache_opration('delete','bypass_permission');
        ...
        $this->data['submodule'] = 'Add '.$this->data["name"]->form_label;
...

    {
        ...
        if (isPostBack()) {
            ...
            $res = $this->db->update('inch_' . $res->form_name, $postData);
        ...
        $this->data['name'] = $this->get_module_for_edit();

        $this->data['submodule'] = 'Edit '.$this->data["name"]->form_label;
...

    {
       ...
       $all_form_data = [];
        ...
        $this->data['form_title']   = "View Form";
        ...
        $this->data['submodule'] = 'View '.$this->data["name"]->form_label;
...

    {
        ...
        if($dynamic_module_name=='bypass_permission')
                ...
                if ($result) {
...

                    {
                        ...
                        foreach ($form_data->elements as $key1 => $value1) {
                            ...
                            $this->db->query("ALTER TABLE inch_".$tablename." DROP ".$value1->name);
    ...
    public function get_module(){
        ...
        $data = strtolower($this->uri->segment(3));

        $this->db->select('form_label,form_name');

        $this->db->from('inch_form');

        $this->db->where('form_name',$data);   
    ...
    public function get_module_for_edit(){

        $data = strtolower($this->uri->segment(5));
    ...
    {   
        ...
        if(empty($text)){  
            ...
            $result =  $this->form_module_mod->dynamic_list_items($module_name,null,null,$is_export,$items_data,$text);

        }

        else{  

            $result =  $this->form_module_mod->ajax_list_items($text,NULL, NULL, NULL, NULL,NULL);
        ...
        unset($column_name['form_id']);

        $data = $this->get_module();

        //pr($data->form_label);die;

        $filename = $data->form_label.''.date('d-m-Y'). ".xls";

        

        $this->export_lib->export($column_name, $result, $filename); 
    ...
    public function import(){  
            ...
            if(isset($_POST['import_btn']) && $_POST['import_btn'] == 'yes') {

                {
...

                  {
                   ...
                   $result =  $this->form_module_mod->dynamic_list_items($module_name,null,null,null);
...

                   {
...

                    {
                        ...
                        foreach($column_name as $key=>$val){ 
                            ...
                            //'$'.$val = $worksheet->getCellByColumnAndRow($i++, $row)->getValue();

                             $data[$key] =  $worksheet->getCellByColumnAndRow($i++, $row)->getValue();
                        ...
                        if(flag==1){
                            ...
                            $this->form_module_mod->import($data,$module_name); 
                   ...
                   redirect(base_url("form_module/dynamic/".$module_name));
...

        {
            ...
            //$this->session->set_flashdata("error_msg","Please select store");
        ...
        $this->data['submodule'] = "Import File";

        

       // pr($this->data);die;ocibindbyname()
