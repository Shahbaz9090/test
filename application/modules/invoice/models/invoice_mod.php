...
class Invoice_mod extends CI_Model {
    ...
    public function add_po_product($order_id)
    {
        ...
        $this->form_validation->set_rules('hsn_code', lang('hsn_code'), 'trim|required');
        $this->form_validation->set_rules('make_issue_inch', lang('make_issue_inch'), 'trim|required');
    ...
    public function change_cfit_name($cfit_id=NULL, $enquiry_id=NULL)
    {
        if($cfit_id && $enquiry_id)
        {
            ...
            return $this->db->update($this->table, ['cfit_name'=>$cfit_id]);
            // echo $this->db->last_query();die;
        }
    }
