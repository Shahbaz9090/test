...
class Mail_mod extends CI_Model {
    ...
    public function get_mail_list($type, $msg_type='inbox', $id='', $count='', $limit='', $start='')
    {
        ...
        {   
			$where = "( CONCAT(cc_email, to_email, from_email, from_name, to_name, reply_name, subject, message) LIKE '%".$search."%' OR FIND_IN_SET('".$search."', tags) )";
            $this->db->where($where);
        }
    ...
    public function perPage($user_id){    		
