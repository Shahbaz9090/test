...
class Sales_Pcb_mod extends CI_Model {
   ...
   public $per_page  = "per_page";
    ...
    {  
        ...
        $this->db->where('leads_sales_pcb.id',$id);
        $query = $this->db->get();
    ...
    function ajax_list_items($text, $limit, $offset, $order_by='id', $order='desc',$user='') {
        ...
        if (!empty($text)) {
            ...
            if(is_numeric($text)){
                ...
                // $this->db->like("LOWER(CONCAT($this->company.name,' ',$this->table.vendor_code,' ',utu.first_name,' ',utu.last_name,' ',$this->table.))", $text);
    ...
    function update($id = NULL)
    {
		...
		$company_name = $this->input->post('company_text',TRUE);
	  ...
	  // $this->form_validation->set_rules('note', lang('note'), 'trim|required');
  	    $this->form_validation->set_rules('vendor_code', lang('vendor_code'), 'trim|required');
		//$st = $this->is_unique_vendor_code($this->input->post('company_text',TRUE));
  	    //pr($st);die; 
	...
	public function is_unique_vendor_code($company_text) {
		//pr($company_text);die;
        //$database=DEFAULT_DATABASE;
