...
class Service_Dcs_mod extends CI_Model {

    public $per_page  = "per_page";
    var $table = "leads_service_dcs";
    ...
    var $lead_service_pcb_others = "lead_service_dcs_others";
    var $lead_service_dcs_common = "lead_service_dcs_common";
    var $disqualify_leads = "disqualify_leads";
    var $company = "companies";
    var $company_contacts = "companies_contact";
    var $lead_service_dcs_notes = "lead_service_dcs_notes";
    var $users = "users";
    ...
    function add()
    {
      // pr($_POST);die;
		...
		set_common_insert_value();    
		$this->db->insert('lead_service_dcs_notes',$note); 
        /*Start save client multiple  document*/
        $leads_service_dcs_doc = [];
        if(isset($_FILES['document']) && !empty($_FILES['document']['name']))
        {
            ...
            foreach ($_FILES['document']['name'] as $document_key => $document_value) {
                ...
                if(isset($fileData['success'])){
                    $leads_service_dcs_doc[$document_key]['filename']	= $fileData['success']['file_name'];   
                    $leads_service_dcs_doc[$document_key]['leads_service_dcs_id']   = $id;   
                }
            }
            
            if(!empty($leads_service_dcs_doc))
            {
                $this->db->insert_batch('leads_service_dcs_doc', $leads_service_dcs_doc);
            }
        }
		$this->db->where('id',$id);
		...
		//$note = $_POST['note'];
          if($note){
             ...
             //pr($noteArray);die;
			$inser_id=$this->db->insert($this->lead_service_dcs_notes,$noteArray);*/
    ...
    {  
		...
		$this->db->select('leads_service_dcs.*,leads_service_dcs.id as main_id,companies.name as companys_name,companies.industry as comp_industry,companies.*,cont.*,cont.name as contact_person_name,lnd.note as lead_service_dcs_notes,ins.name as industry_name,con.country_name as country_name,st.state_name as state_name,ct.city_name as city_name,dept.name as department_name,desg.name as designation_name');
		$this->db->join('companies','companies.id =leads_service_dcs.company_name','Inner');
		$this->db->join('companies_contact as cont','cont.company_id =companies.id','left');
		$this->db->join('lead_service_dcs_notes as lnd','lnd.lead_id =leads_service_dcs.id','INNER');
		...
		$this->db->from("leads_service_dcs");
        $this->db->where('leads_service_dcs.id',$id);
        $query = $this->db->get();
		...
		$this->db->select('*');
        $this->db->where('leads_service_dcs_id', $id);
        $leads_service_dcs_doc = $this->db->get("leads_service_dcs_doc")->result();
        @$query->leads_service_dcs_doc  =   $leads_service_dcs_doc;
	...
	function get_notes($id = NULL)
    {
        ...
        $this->db->select("lead_service_dcs_notes.*,users.first_name,users.last_name");
		$this->db->join('users','users.id =lead_service_dcs_notes.added_by');
		$this->db->from("lead_service_dcs_notes");
		$this->db->where('lead_id',$id);
       ...
       /* $fileData=$this->uploadDoc($_FILES['document']);
        ...
        if(isset($_FILES['document']) && !empty($_FILES['document']['name']))
        {
            ...
            foreach ($_FILES['document']['name'] as $document_key => $document_value) {
                ...
                if(isset($fileData['success'])){
                    $leads_service_dcs_doc[$document_key]['filename']	= $fileData['success']['file_name'];   
                    $leads_service_dcs_doc[$document_key]['leads_service_dcs_id']   = $lead_id;   
                }
            }
		...
		$note = $_POST['note'];
          if($note){
				...
				/*End of putting company data into table in form of json*/
				$note_up['note'] = $note;
				//$this->db->update($this->lead_service_dcs_notes,$note_up); 
    ...
    function addCommon($data = array()) {
		//pr($data);die;
        $leadOthers_plc = _otherFieldsData('lead_service_dcs_common_plc', 'lead_id', $data['lead_id']);
        $leadOthers_actuator = _otherFieldsData('lead_service_dcs_common_actuator', 'lead_id', $data['lead_id']);
        $leadOthers_vfd = _otherFieldsData('lead_service_dcs_common_vfd', 'lead_id', $data['lead_id']);
			 ...
			 $len_plc = $data['open_plc_varient_count'];
				...
				$varient_menu_plc['modified'] = _dateTime();
					if(!empty($data['plc_dcs_make'][$i]) && !empty($data['plc_dcs_qty'][$i])){
						...
						//pr($varient_menu_plc);die;
						
						$this->db->where('id', $plc_varient_id_up);
						$this->db->update( 'lead_service_dcs_common_plc', $varient_menu_plc);
						//pr($this->db->last_query());die;
					}else{
					
						$this->db->where('id', $plc_varient_id_up);
						$this->db->delete('lead_service_dcs_common_plc');
					}
				}else{
					$varient_menu_plc['lead_id'] = $data['lead_id'];
					...
					$varient_menu_plc['plc_dcs_qty'] = $data['plc_dcs_qty'][$i];
					if(!empty($data['plc_dcs_make'][$i]) && !empty($data['plc_dcs_qty'][$i])){
					$this->db->insert('lead_service_dcs_common_plc', $varient_menu_plc);
					}
				}
			 ...
			 $len_actuator = $data['open_actuator_varient_count'];
				...
				$varient_menu_actuator['modified'] = _dateTime();
					if(!empty($data['actuator_make'][$i]) && !empty($data['actuator_qty'][$i])){
						...
						$this->db->update( 'lead_service_dcs_common_actuator', $varient_menu_actuator);
					}else{
					
						$this->db->where('id', $actuator_varient_id_up);
						$this->db->delete('lead_service_dcs_common_actuator');
					}
				}else{
					$varient_menu_actuator['lead_id'] = $data['lead_id'];
					...
					$varient_menu_actuator['actuator_qty'] = $data['actuator_qty'][$i];
					if(!empty($data['actuator_make'][$i]) && !empty($data['actuator_qty'][$i])){
					$this->db->insert('lead_service_dcs_common_actuator', $varient_menu_actuator);
					}
				}
		 ...
		 if ($leadOthers_vfd) {
			 ...
			 $len_vfd = $data['open_vfd_varient_count'];
				...
				if(!empty($data['vfd_make'][$i]) && !empty($data['vfd_qty'][$i])){
					...
					$this->db->update('lead_service_dcs_common_vfd', $varient_menu_vfd);
				}else{
					//echo "ayaaa";die;
					$this->db->where('id', $vfd_varient_id_up);
					$this->db->delete('lead_service_dcs_common_vfd');
				}
				}else{
					$varient_menu_vfd['lead_id'] = $data['lead_id'];
					...
					$varient_menu_vfd['vfd_qty'] = $data['vfd_qty'][$i];
					if(!empty($data['vfd_make'][$i]) && !empty($data['vfd_qty'][$i])){
						$this->db->insert('lead_service_dcs_common_vfd', $varient_menu_vfd);
					}
				}
			}
	...
	public function check_vendor_code_existance($vendor_code) {
        ...
        $query=$this->db->query("SELECT id FROM leads_service_dcs WHERE vendor_code='$vendor_code'");
    ...
    function disqualified_notes() {
        ...
        $this->db->order_by('id', 'desc');
		$query = $this->db->get('leads_service_dcs');
		//pr($this->db->last_query());die;
        if ($query->num_rows()) {
			return $query->result();
        }
	...
	public function check_name_existance($company_text,$id=null) {
       ...
       // pr($vendor_code);die;
        if(empty($id)){  //pr("yess");
            ...
            $result = $query->row();
            //pr($result);die;
            $query1 = $this->db->query("SELECT company_name FROM leads_service_dcs WHERE leads_service_dcs.company_name=".$result->id);
            
        }else{  //pr("no");
          $query=$this->db->query("SELECT id FROM companies WHERE companies.name='$company_text'");
            $result = $query->row();
            //pr($result);die;
            $query1 = $this->db->query("SELECT company_name FROM leads_service_dcs WHERE leads_service_dcs.id !='$id' && leads_service_dcs.company_name=".$result->id);
        }
    //pr($this->db->last_query());die;
	...
	public function is_unique_vendor_code($vendor_code) {
		...
		//pr($id);die;
        $query=$this->db->query("SELECT id FROM leads_service_dcs WHERE vendor_code='$vendor_code' AND id!=".$id);
        // echo $this->db->last_query();exit;
        $count=$query->num_rows();		
        if ($count)
        {
