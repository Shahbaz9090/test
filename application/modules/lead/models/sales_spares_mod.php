...
class Sales_Spares_mod extends CI_Model {
   ...
   // public $table     = "companies";
    ...
    function add()
    {
      // pr($_POST);die;
		...
		set_common_insert_value();    
		$this->db->insert('leads_sales_spares_notes',$note);   
        ...
        if(isset($_FILES['document']) && !empty($_FILES['document']['name']))
        {
            ...
            foreach ($_FILES['document']['name'] as $document_key => $document_value) {
                ...
                if(isset($fileData['success'])){
                    $sales_spares_doc[$document_key]['filename']	= $fileData['success']['file_name'];   
                    $sales_spares_doc[$document_key]['leads_sales_spares_id']   = $id;   
                }
            }
            
            if(!empty($sales_spares_doc))
            {
                $this->db->insert_batch('leads_sales_spares_doc', $sales_spares_doc);
            }
        }
		...
		$note = $_POST['note'];
          if($note){
             ...
             //pr($noteArray);die;
			$inser_id=$this->db->insert($this->lead_notes,$noteArray);
    ...
    {  
		...
		$this->db->select('leads_sales_spares.*,leads_sales_spares.id as main_id,companies.name as companys_name,companies.industry as comp_industry,companies.*,companies.id as company_id,cont.*,cont.id as contact_id,cont.name as contact_person_name,lnd.note as lead_notes,ins.name as industry_name,con.country_name as country_name,st.state_name as state_name,ct.city_name as city_name,dept.name as department_name,desg.name as designation_name');
		$this->db->join('companies','companies.id =leads_sales_spares.company_name','INNER');
		$this->db->join('companies_contact as cont','cont.company_id =companies.id','left');
		$this->db->join('lead_notes as lnd','lnd.lead_id =leads_sales_spares.id','Inner');
		...
		$this->db->from("leads_sales_spares ");
        $this->db->where('leads_sales_spares.id',$id);
        $query = $this->db->get();
        ...
        $menu_varient_other = $this->db->get("lead_others")->result();
		@$query->lead_others	=	$menu_varient_other;
		
		$this->db->select('*');
        $this->db->where('leads_sales_spares_id', $id);
        $leads_sales_spares_doc = $this->db->get("leads_sales_spares_doc")->result();
        @$query->leads_sales_spares_doc  =   $leads_sales_spares_doc;
    ...
    /*function get_common(){
		...
		$common_id = $this->session->userdata("key");
		//pr($common_id );die;
		$this->db->select('*');
        $this->db->where('lead_id', $common_id);
        $menu_varient_plc = $this->db->get("lead_sales_pcb_common_plc")->result();
    ...
    function get_notes($id = NULL)
    {
        ...
        $this->db->select("leads_sales_spares_notes.*,users.first_name,users.last_name");
		$this->db->join('users','users.id =leads_sales_spares_notes.added_by');
		$this->db->from("leads_sales_spares_notes");
		$this->db->where('lead_id',$id);
    ...
    function ajax_list_items($text, $limit, $offset, $order_by='id', $order='desc',$user='') {
        ...
        $this->db->select("SQL_CALC_FOUND_ROWS $this->table.*, CASE WHEN $this->table.lead_status=0 THEN 'Initiated' ELSE 'Initiated' END AS lead_status,utu.first_name AS USER_NAME,$this->company_contacts.name as contact_name, concat(utu.first_name,' ',utu.last_name) as added_by, $this->company.name as company_name", false);
        if (!empty($text)) {
			$text	=	trim($text);
            if(is_numeric($text)){
        ...
        $this->db->join($this->company, "$this->company.id=$this->table.company_name",'inner');
        $this->db->join($this->company_contacts, "$this->company_contacts.company_id=$this->company.id",'left');
        ...
        if($order_by =='follow_up_date'){
            ...
            $this->db->order_by($this->table.'.'.$order_by, $order);
        }
		
        $this->db->limit($limit, $offset);

        if ($status == 'archieved') {
            $this->db->where("$this->table.lead_status", '1');
        } else {
            $this->db->where("$this->table.lead_status", '0');
    ...
    {	
        ...
        {
            //set_flashdata("error",validation_errors());
            // var_dump(form_error('vendor_code'));die;
            return FALSE;
        }
		// elseif ($st == TRUE)
  //       {	//pr("yaa");die;
  ...
  //       } 
        ...
        // set_common_insert_value();        
        $r = $this->db->update($this->table,$lead_add);
		...
		set_common_insert_value();    
		$this->db->insert('leads_sales_spares_notes',$lead_upd);
		}
		/*Start save client multiple  document*/
        $sales_spares_doc = [];
        if(isset($_FILES['document']) && !empty($_FILES['document']['name']))
        {
            ...
            foreach ($_FILES['document']['name'] as $document_key => $document_value) {
                ...
                if(isset($fileData['success'])){
                    $sales_spares_doc[$document_key]['filename']	= $fileData['success']['file_name'];   
                    $sales_spares_doc[$document_key]['leads_sales_spares_id']   = $lead_id;   
                }
            }
		...
		//$r = $this->db->update($this->table,$company_up);
		//$note = $_POST['note'];
      	//if($note){
			...
			/*End of putting company data into table in form of json*/
			//$note_up['note'] = $note;
			//$this->db->update($this->lead_notes,$note_up); 
	  	//}
	...
	function uploadDoc($file_arr) {
		...
		if (@$file_arr['name'] != '') {
            ...
            $name = md5(time());
            $file_name = $name . "." . $ext;
			$folder_doc = './upload/sales_spares/';
    ...
    function addOthers($data = array()) {
		//pr($data);die;
        $leadOthers = _otherFieldsData($this->lead_others, 'lead_id', $data['lead_id']);
		//pr($leadOthers);die;
		//pr($leadOthers);die;
        ...
        if ($leadOthers) {
            ...
            for ($i = 0; $i < $len_macho; $i++) {
               ...
               // $varient_menu['modified'] = _dateTime();
                ...
                $varient_menu['coal_mill_make'] = $data['coal_mill_make'][$i];
                $varient_menu['capacity_of_unit'] = $data['capacity_of_unit'][$i];
                $varient_menu['id_fan'] = $data['id_fan'][$i];
                $varient_menu['epc_company'] = $data['epc_company'][$i];
                $varient_menu['id_fan_motor_make'] = $data['id_fan_motor_make'][$i];
                $varient_menu['cod_month'] = $data['cod_month'][$i];
                $varient_menu['cod_year'] = $data['cod_year'][$i];
                $varient_menu['fd_fan_make'] = $data['fd_fan_make'][$i];
                $varient_menu['boiler_make'] = $data['boiler_make'][$i];
                $varient_menu['fd_fan_motor_make'] = $data['fd_fan_motor_make'][$i];
                $varient_menu['turbine_make'] = $data['turbine_make'][$i];
                $varient_menu['pa_fan_make'] = $data['pa_fan_make'][$i];
                $varient_menu['generator_make'] = $data['generator_make'][$i];
                $varient_menu['pa_fan_motor_make'] = $data['pa_fan_motor_make'][$i];
                $varient_menu['bfpt_make'] = $data['bfpt_make'][$i];
                $varient_menu['esp_make'] = $data['esp_make'][$i];
                $varient_menu['bfpm_make'] = $data['bfpm_make'][$i];
                $varient_menu['aph_make'] = $data['aph_make'][$i];
                $varient_menu['bfp_make'] = $data['bfp_make'][$i];
                $varient_menu['vibration_system'] = $data['vibration_system'][$i];
				$varient_menu['bfbp_make'] = $data['bfbp_make'][$i];
                $varient_menu['btld_make'] = $data['btld_make'][$i];
                $varient_menu['cep_make'] = $data['cep_make'][$i];
                $varient_menu['coal_feeder_make'] = $data['coal_feeder_make'][$i];
                $varient_menu['cep_motor_make'] = $data['cep_motor_make'][$i];
                $varient_menu['flame_scanner_make'] = $data['flame_scanner_make'][$i];
				
					if(!empty($data['no_of_unit'][$i]) && !empty($data['coal_mill_make'][$i])){
					$this->db->where('id', $macho_varient_id_up);
					$this->db->update($this->lead_others, $varient_menu);
					}else{
						$this->db->where('id', $macho_varient_id_up);
						$this->db->delete($this->lead_others);
					}
				}else{
				$varient_menu['lead_id'] = $data['lead_id'];
                ...
                $varient_menu['flame_scanner_make'] = $data['flame_scanner_make'][$i];
				if(!empty($data['no_of_unit'][$i]) && !empty($data['coal_mill_make'][$i])){
					$this->db->insert($this->lead_others, $varient_menu);
					}
				}
    ...
    function addCommon($data = array()) {
		//pr($data);die;
        $leadOthers_plc = _otherFieldsData('lead_common_plc', 'lead_id', $data['lead_id']);
        $leadOthers_actuator = _otherFieldsData('lead_common_actuator', 'lead_id', $data['lead_id']);
        $leadOthers_vdf = _otherFieldsData('lead_common_vfd', 'lead_id', $data['lead_id']);
			 ...
			 $len_plc = $data['open_plc_varient_count'];
				...
				$varient_menu_plc['modified'] = _dateTime();
					if(!empty($data['plc_dcs_make'][$i]) && !empty($data['plc_dcs_qty'][$i])){
						...
						$this->db->update( 'lead_common_plc', $varient_menu_plc);
					}else{
					
						$this->db->where('id', $plc_varient_id_up);
						$this->db->delete('lead_common_plc');
					}
				}else{
					$varient_menu_plc['lead_id'] = $data['lead_id'];
					...
					$varient_menu_plc['plc_dcs_qty'] = $data['plc_dcs_qty'][$i];
					if(!empty($data['plc_dcs_make'][$i]) && !empty($data['plc_dcs_qty'][$i])){
					$this->db->insert('lead_common_plc', $varient_menu_plc);
					}
				}
			 ...
			 //pr($len_actuator);die;
				...
				//pr($actuator_varient_id_up);die;
					if(!empty($data['actuator_make'][$i]) && !empty($data['actuator_qty'][$i])){
						...
						$this->db->update( 'lead_common_actuator', $varient_menu_actuator);
					}else{
						
						$this->db->where('id', $actuator_varient_id_up);
						$this->db->delete('lead_common_actuator');
					}
				}else{
					$varient_menu_actuator['lead_id'] = $data['lead_id'];
					...
					$varient_menu_actuator['actuator_make'] = $data['actuator_make'][$i];
					$varient_menu_actuator['actuator_qty'] = $data['actuator_qty'][$i];
					//pr($varient_menu_actuator);die;
					if(!empty($data['actuator_make'][$i]) && !empty($data['actuator_qty'][$i])){
					$this->db->insert('lead_common_actuator', $varient_menu_actuator);
					}
				}
		 ...
		 if ($leadOthers_vdf) {
			 ...
			 $len_vfd = $data['open_vfd_varient_count'];
				...
				if(!empty($data['vfd_make'][$i]) && !empty($data['vfd_qty'][$i])){
					...
					$this->db->update('lead_common_vfd', $varient_menu_vfd);
				}else{
					//echo "ayaaa";die;
					$this->db->where('id', $vfd_varient_id_up);
					$this->db->delete('lead_common_vfd');
				}
				}else{
					$varient_menu_vfd['lead_id'] = $data['lead_id'];
					...
					$varient_menu_vfd['vfd_qty'] = $data['vfd_qty'][$i];
					if(!empty($data['vfd_make'][$i]) && !empty($data['vfd_qty'][$i])){
						$this->db->insert('lead_common_vfd', $varient_menu_vfd);
					}
				}
			}
    ...
    public function check_vendor_code_existance($vendor_code) {
        ...
        $query=$this->db->query("SELECT id FROM leads_sales_spares WHERE vendor_code='$vendor_code'");
     ...
     * addCompanyContact
     *
     * This function add Cmpany contact deatil
     * 
     * @access	public
     * @return	TRUE or FALSE 
     */
    function addCompanyContact($data = array()) {
        $id = $data['contact_id'];
    ...
    function disqualified_notes() {
        ...
        $this->db->order_by('id', 'desc');
		$query = $this->db->get('leads_sales_spares');
		//pr($this->db->last_query());die;
        if ($query->num_rows()) {
			return $query->result();
        }
	...
	public function check_name_existance($company_text,$id=null) {
       ...
       // pr($vendor_code);die;
        if(empty($id)){  //pr("yess");
            ...
            $result = $query->row();
            //pr($result);die;
            $query1 = $this->db->query("SELECT company_name FROM leads_sales_spares WHERE leads_sales_spares.company_name=".$result->id);
            
        }else{  //pr("no");
        	$query=$this->db->query("SELECT id FROM companies WHERE companies.name='$company_text'");
            $result = $query->row();
            //pr($result);die;
            $query1 = $this->db->query("SELECT company_name FROM leads_sales_spares WHERE leads_sales_spares.id !='$id' && leads_sales_spares.company_name=".$result->id);
        }
		//pr($this->db->last_query());die;
	...
	public function is_unique_vendor_code($vendor_code) {
		...
		//pr($id);die;
        $query=$this->db->query("SELECT id FROM leads_sales_spares WHERE vendor_code='$vendor_code' AND id!=".$id);
        // echo $this->db->last_query();exit;
        $count=$query->num_rows();		
        if ($count)
        {
