	...
	<div class="span12">
      ...
      <?php echo get_flashdata();?>
		<div class="box">
          ...
          <?php if($row->leads_service_dcs_doc){ ?>
					<div class="span6" style="border: gray solid 1px;">
						...
						<div class="document-wrapper">
							<div class="document-list">
								<ul>
									<?php 
									if(isset($row->leads_service_dcs_doc) && !empty($row->leads_service_dcs_doc))
									{
										foreach ($row->leads_service_dcs_doc as $leads_service_dcs_doc_key => $leads_service_dcs_doc_value) {
											$ext = $leads_service_dcs_doc_value->filename;
											$ext = substr($ext, strripos($ext, '.')+1);
											if($ext=='jpg' || $ext=='jpeg' || $ext == 'png'){?>
											<li>
												<i><img style="height:12px;width:12px;padding-right:3px;position: absolute;margin: auto;bottom: 0;top: 0;" src="<?=base_url('upload/service_dcs/'.$leads_service_dcs_doc_value->filename)?>"/></i>
												<span class="file-title"><?php echo $leads_service_dcs_doc_value->filename ?></span>
												<span data-id="<?=$leads_service_dcs_doc_value->id?>" onclick="return removeFile(this)" class="remove-image-box"><i class="fa fa-trash-o pull-right"></i></span>
												<a download href="<?=base_url('upload/service_dcs/'.$leads_service_dcs_doc_value->filename)?>" class="file-action file-delete"><i class="fa fa-download pull-right"></i></a>
											</li>
          ...
          <?php } ?>				
						...
						<span class="document_previews_old">
							...
							if(isset($row->leads_service_dcs_doc) && !empty($row->leads_service_dcs_doc))
							{
								foreach ($row->leads_service_dcs_doc as $leads_service_dcs_doc_key => $leads_service_dcs_doc_value) {
									...
									{?>
										<span class="img-box">
											<a download href="<?=base_url('upload/service_dcs/'.$leads_service_dcs_doc_value->filename)?>"><img style="height:20px;width:30px;padding-right:3px" src="<?=base_url('upload/service_dcs/'.$leads_service_dcs_doc_value->filename)?>"/></a>
											<span data-id="<?=$leads_service_dcs_doc_value->id?>" onclick="return removeFile(this)" class="remove-image-box">&times;</span>
										</span>

									<?php } else{?>
										<span class="img-box">
											<a download href="<?=base_url('upload/service_dcs/'.$leads_service_dcs_doc_value->filename)?>"><img style="height:20px;width:30px;padding-right:3px" src="<?=base_url('assets/images/file_icon.png')?>"/></a>
											<span data-id="<?=$leads_service_dcs_doc_value->id?>" onclick="return removeFile(this)" class="remove-image-box">&times;</span>
										</span>
									<?php }
								}
			...
			<!--------------------End-Documents Upload Info------------------------->	
                ...
                <span id="vendor_error" class="help-block" style="width: auto;"></span>
							</div>
						</div>	
					</div>
				</div>
...
$(document).on('click','.minus_vfd',function(){
    ...
    var action="<?=SITE_PATH?>lead/service_dcs/checkVendorCodeExistence";
...
function removeFile(obj)
{
	...
	{ 
		var leads_service_dcs_doc_id = $(obj).attr('data-id');
	    $.ajax({  
			method:"GET", 
			dataType:'json', 
			url:"<?php echo base_url();?>lead/service_dcs/remove_client_doc",
			data:{leads_service_dcs_doc_id:leads_service_dcs_doc_id},  
			success:function(data)  
			{
			 	if(data.status==1)
			 	{
...
function checkUniqueness(company_name){
    ...
    $("#company_name_existance_error").html("");
      ...
      //alert(id);
      var status = 0;
      var action="<?=SITE_PATH?>lead/service_dcs/checkNameExistence";
