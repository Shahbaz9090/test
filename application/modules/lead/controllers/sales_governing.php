...
class Sales_Governing extends MY_Controller {
   
    private $data 			= array();
    public  $table 			= "leads_sales_governing";
    ...
    function __construct()
    {
        ...
        $this->load->model('sales_governing_mod');
        ...
        $this->lang->load('sales_governing', get_site_language());
               
        $this->data['head']['title'] = "Sales Governing";
        $this->data['readonly'] = NULL;
        $this->data['base_url'] = base_url("lead/sales_governing");
		...
		$this->data['module'] = 'Installation Base/Sales Governing';
        $this->data['module_link'] = base_url("lead/sales_governing")."/list_items";
	...
	public function add()
	{
	   ...
	   if(isPostBack())
       {
            $id = $this->sales_governing_mod->add();
            if($id)
            {
	    ...
	    $views[] = "sales_governing/lead_form";
		
		$table_data = $this->sales_governing_mod->get_table_name();
		//pr($table_data);die;
		foreach($table_data as $key=>$val){
		   ...
		   if($val->form_name=='vib_system_make_model'){
			$table_name = 'inch_vib_system_make_model';
			$form_data ='';
			$this->data['vib_system_make_model'] = dynamic_module_master($table_name,$form_data);
		   }
		   if($val->form_name=='ext_system_make_model'){
			$table_name = 'inch_ext_system_make_model';
			$form_data ='';
			$this->data['ext_system_make_model'] = dynamic_module_master($table_name,$form_data);
		   }
		   if($val->form_name=='governing_turbine_make'){
			$table_name = 'inch_governing_turbine_make';
			$form_data ='';
			$this->data['governing_turbine_make'] = dynamic_module_master($table_name,$form_data);
		   }
		   if($val->form_name=='elec_governor_make_model'){
			$table_name = 'inch_elec_governor_make_model';
			$form_data ='';
			$this->data['elec_governor_make_model'] = dynamic_module_master($table_name,$form_data);
		   }
		   if($val->form_name=='governing_generator_make'){
			$table_name = 'inch_governing_generator_make';
			$form_data ='';
			$this->data['governing_generator_make'] = dynamic_module_master($table_name,$form_data);
		   }
		   if($val->form_name=='i_h_actuator_make_model'){
			$table_name = 'inch_i_h_actuator_make_model';
			$form_data ='';
			$this->data['i_h_actuator_make_model'] = dynamic_module_master($table_name,$form_data);
		   }
		...
		//echo 'sckd';die;
	  $result = $this->sales_governing_mod->get_common();
      $this->data['row'] = $result;
	   
	  $this->data['submodule'] = 'Add Sales Governing';
	  $this->data['industry'] = $this->sales_governing_mod->get_industry();
	...
	{        
       $result = $this->sales_governing_mod->get($id);
       $this->data['result'] = $result;
       $this->data['readonly'] = 'readonly="true"';
       $this->data['action'] = "view";
       $views[] = "sales_governing/view_lead"; 
	   $table_data = $this->sales_governing_mod->get_table_name();
	   foreach($table_data as $key=>$val){
			if($val->form_name=='plc_dcs_make'){
			$table_name = 'inch_plc_dcs_make';
		...
		$note = $this->sales_governing_mod->get_notes($id);
	   $this->data['note'] = $note;
	   $appoitment = $this->sales_governing_mod->appointment_data();
	   $disqualified_notes = $this->sales_governing_mod->disqualified_notes();
	   $notes = $this->sales_governing_mod->notes();
	   $product = $this->sales_governing_mod->product();
	   $service = $this->sales_governing_mod->service();
	   $this->data['service'] = $service;
	   $this->data['product'] = $product;
	   $this->data['notes'] = $notes;
       $this->data['appoitment'] = $appoitment;
       $this->data['disqualified_notes'] = $disqualified_notes;
	   $this->data['submodule'] = 'View Sales Governing';
	...
	{   
		...
		if(check_own_all_permission( ['id'=>$id], $this->table)==false)
        {
            ...
            return redirect(base_url('lead/sales_governing/list_items'));exit;
        }	   
       if(isPostBack())
       {
           $r = $this->sales_governing_mod->update($id);
	        if($r)
	        {
	            redirect($this->data['base_url']."/list_items/");
			}
	        else{
	            $this->data['error_msg']="Oops !! This Client Name does not exists.";
			}
       }
	   ...
	   //pr($result);die;
	   $notes = $this->sales_governing_mod->get_notes($id);
	   $this->data['notes'] = $notes;
	   $this->data['row'] = $result;
	   $this->data['industry'] = $this->sales_governing_mod->get_industry();
	  $this->data['country'] = get_country_from_master();
      ...
      $this->data['designation'] = get_all_designation_from_masters();
	   ...
	   $this->data['submodule'] = 'Edit Sales Governing';
	   //pr($this->data);die;
	   view_load($views,$this->data);
	...
	{  
		...
		$views[] = "sales_governing/lead_list";
        $this->data['title'] = lang('list_title');
        $this->data['place_holder'] = "Enter filter terms here";        
        $this->data['action'] = "list";

        $this->data['grid']['cols'] = $this->sales_governing_mod->get_flexigrid_cols();
        ...
        $order_by='id';
        $order='desc';
        $result =  $this->sales_governing_mod->ajax_list_items($text, $limit, $offset, $order_by, $order,$user);
		//pr($result['result']);
		$priority = _priority();
        $referalSource = _referalSourceList();
        foreach ($result['result'] as $row) { 
            if (!empty($row->display_id)) {
                $row->display_id = '<a href="'.base_url().'lead/sales_governing/view/'.$row->id.'">#'.$row->display_id.'</a>';//'#'.$v[$val['name']];
        ...
        $this->data['submodule'] = 'Sales Governing';
        view_load($views, $this->data);
	}
	...
	{ 
		...
		$perPage = $this->sales_governing_mod->perPage($user->id);
        ...
        if($perPage) {
            ...
            $pageArr = array(
                ...
                'user_id' => $user->id);
            $this->sales_governing_mod->insert_perPage($pageArr);
        }
        ...
        $data = $this->sales_governing_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
    ...
    //             }
        ...
        $data['grid']['cols'] = $this->sales_governing_mod->get_flexigrid_cols();
  ...
  // ------------------------------------------------------------------------
    ...
    public function checkVendorCodeExistence() {
        ...
        $result = $this->sales_governing_mod->check_vendor_code_existance($vendor_code);
        //pr($result);die;
        if ($result == true)
            echo true;
    }    
     ...
     */
	  ...
	  return redirect(base_url('lead/sales_governing/view/'.$_POST['lead_id']));
  }
     /**
     * Qualified Sales governing
     *
     * This function show Sales governing
     * 
     * @access	public
     * @return  json array 
     */
	public function qualified_form() {
	  ...
	  $this->db->where('id',$id);
	  $this->db->update('leads_sales_governing', $data);
	  //pr($this->db->last_query());die;
	  return redirect(base_url('lead/sales_governing/list_items'));
  }

   /**
     * Diequalified Sales governing
     *
     * This function show Sales governing
     * 
     * @access	public
	...
	public function remove_client_doc() {
		...
		if(isset($_GET['leads_sales_governing_doc_id']) && !empty($_GET['leads_sales_governing_doc_id']) && is_numeric($_GET['leads_sales_governing_doc_id']))
		{
			$leads_sales_governing_doc_id = $_GET['leads_sales_governing_doc_id'];
			$this->db->select('filename');
			$this->db->where('id',$leads_sales_governing_doc_id);
			$record = $this->db->get("leads_sales_governing_doc")->row();
			if(isset($record->filename) && !empty($record->filename))
			{
				unlink(FCPATH.'upload/sales_governing/'.$record->filename);
				$this->db->where('id',$leads_sales_governing_doc_id);
				$this->db->delete('leads_sales_governing_doc');
	...
	public function checkNameExistence() {
        ...
        $id = $this->input->get_post('id', true);
		$result = $this->sales_governing_mod->check_name_existance($company_text,$id);
        //pr($result);die;
        if ($result == true)
            echo true;
    }
