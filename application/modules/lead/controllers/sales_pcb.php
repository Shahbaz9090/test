...
class Sales_Pcb extends MY_Controller {
   
    private $data 			= array();
    public $table 			= "leads_sales_pcb";
    ...
    function __construct()
    {
        ...
        $this->lang->load('sales_pcb', get_site_language());
               
        $this->data['head']['title'] = "Sales PCB/Fanuc";
        $this->data['readonly'] = NULL;
        $this->data['base_url'] = base_url("lead/sales_pcb");
        $this->export_limit = $this->config->item('export_limit');
        $this->delete_limit = $this->config->item('delete_limit');
        $this->table_name   = "leads_sales_pcb";

		$this->data['module'] = 'Installation Base/Sales PCB/Fanuc';
        $this->data['module_link'] = base_url("lead/sales_pcb")."/list_items";
	...
	public function add()
	{
	   ...
	   $views[] = "sales_pcb/lead_form";
	  $table_data = $this->sales_pcb_mod->get_table_name();
		//pr($table_data);die;
		foreach($table_data as $key=>$val){
		   ...
		   if($val->form_name=='type_make_machine'){
			$table_name = 'inch_type_make_machine';
			$form_data ='';
			$this->data['type_make_machine'] = dynamic_module_master($table_name,$form_data);
		   }
		   if($val->form_name=='make_model_controller'){
			$table_name = 'inch_make_model_controller';
			$form_data ='';
			$this->data['make_model_controller'] = dynamic_module_master($table_name,$form_data);
		   }
	  ...
	  /*$result = $this->sales_pcb_mod->get_common();
      $this->data['row'] = $result;*/
	  $this->data['submodule'] = 'Add Sales PCB/Fanuc';
	  $this->data['industry'] = $this->sales_pcb_mod->get_industry();
	...
	{        
       $result = $this->sales_pcb_mod->get($id);
       $this->data['result'] = $result;
       $this->data['readonly'] = 'readonly="true"';
       $this->data['action'] = "view";
       $views[] = "sales_pcb/view_lead"; 
	   $appoitment = $this->sales_pcb_mod->appointment_data();
	   ...
	   $table_data = $this->sales_pcb_mod->get_table_name();
		...
		$note = $this->sales_pcb_mod->get_notes($id);
	   $this->data['note'] = $note;
	   $appoitment = $this->sales_pcb_mod->appointment_data();
	   $disqualified_notes = $this->sales_pcb_mod->disqualified_notes();
	   $notes = $this->sales_pcb_mod->notes();
	   $product = $this->sales_pcb_mod->product();
	   $service = $this->sales_pcb_mod->service();
	   $this->data['service'] = $service;
	   $this->data['product'] = $product;
	   $this->data['notes'] = $notes;
       $this->data['appoitment'] = $appoitment;
       $this->data['disqualified_notes'] = $disqualified_notes;
	   $this->data['submodule'] = 'View Sales PCB/Fanuc';
	...
	{ 
		...
		if(check_own_all_permission( ['id'=>$id], $this->table)==false)
        {
            ...
            return redirect(base_url('lead/sales_pcb/list_items'));exit;
        }  	   
       if(isPostBack())
       {
           $r = $this->sales_pcb_mod->update($id);
		   //pr($r);die;
            if($r){
                redirect($this->data['base_url']."/list_items/");
			}
	   ...
	   $table_data = $this->sales_pcb_mod->get_table_name();
		...
		foreach($table_data as $key=>$val){
		 ...
		 $notes = $this->sales_pcb_mod->get_notes($id);
	   $this->data['notes'] = $notes;
	   $this->data['row'] = $result;
	   //pr($result);die;
	   $this->data['industry'] = $this->sales_pcb_mod->get_industry();
      ...
      $this->data['designation'] = get_all_designation_from_masters();
	   ...
	   $this->data['submodule'] = 'Edit Sales PCB/Fanuc';
	   //pr($this->data);die;
	   view_load($views,$this->data);
	...
	{  
		...
		$views[] = "sales_pcb/lead_list";
        $this->data['title'] = lang('list_title');
        $this->data['place_holder'] = "Enter filter terms here";        
        $this->data['action'] = "list";

        $this->data['grid']['cols'] = $this->sales_pcb_mod->get_flexigrid_cols();
        ...
        $order_by='id';
        $order='desc';
        $result =  $this->sales_pcb_mod->ajax_list_items($text, $limit, $offset, $order_by, $order,$user);
		//pr($result['result']);
		$priority = _priority();
        $referalSource = _referalSourceList();
        foreach ($result['result'] as $row) { 
            if (!empty($row->display_id)) {
                $row->display_id = '<a href="'.base_url().'lead/sales_pcb/view/'.$row->id.'">#'.$row->display_id.'</a>';//'#'.$v[$val['name']];
			...
			if (!empty($row->assigned_telemarketer)) { // for assign telemarketer value
				...
				if ($row->created==0 || $row->created) {
                   $row->created = date("d-M-Y ",strtotime($row->created_tiime));
                    
                }
				if ($row->modified_time==0 || $row->modified_time) {
                   $row->modified_time = date("d/m/Y ",strtotime($row->created_time));
        ...
        $this->data['submodule'] = 'Sales PCB/Fanuc';
	...
	{ 
		...
		$perPage = $this->sales_pcb_mod->perPage($user->id);
        ...
        if($perPage) {
            ...
            $pageArr = array(
                ...
                'user_id' => $user->id);
            $this->sales_pcb_mod->insert_perPage($pageArr);
        }
        ...
        $data = $this->sales_pcb_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
        ...
        $data['grid']['cols'] = $this->sales_pcb_mod->get_flexigrid_cols();
    ...
    public function export()
    {
       ...
       $data = $this->sales_pcb_mod->export();
  ...
  // ------------------------------------------------------------------------
	...
	public function checkVendorCodeExistence() {
        ...
        $result = $this->sales_pcb_mod->check_vendor_code_existance($vendor_code);
        //pr($result);die;
        if ($result == true)
            echo true;
    }
     ...
     */
	  ...
	  return redirect(base_url('lead/sales_pcb/view/'.$_POST['lead_id']));
  }
     /**
	...
	public function qualified_form() {
	  ...
	  $this->db->where('id',$id);
	  $this->db->update('leads_sales_pcb', $data);
	  //pr($this->db->last_query());die;
	  return redirect(base_url('lead/sales_pcb/list_items'));
  }
   ...
   /**
	...
	public function remove_client_doc() {
		...
		if(isset($_GET['leads_sales_pcb_doc_id']) && !empty($_GET['leads_sales_pcb_doc_id']) && is_numeric($_GET['leads_sales_pcb_doc_id']))
		{
			$leads_sales_pcb_doc_id = $_GET['leads_sales_pcb_doc_id'];
			$this->db->select('filename');
			$this->db->where('id',$leads_sales_pcb_doc_id);
			$record = $this->db->get("leads_sales_pcb_doc")->row();
			if(isset($record->filename) && !empty($record->filename))
			{
				unlink(FCPATH.'upload/sales_pcb/'.$record->filename);
				$this->db->where('id',$leads_sales_pcb_doc_id);
				$this->db->delete('leads_sales_pcb_doc');
	...
	public function checkNameExistence() {
        ...
        $id = $this->input->get_post('id', true);
		$result = $this->sales_pcb_mod->check_name_existance($company_text,$id);
        //pr($result);die;
        if ($result == true)
            echo true;
    }
