...
class Sales_Spares extends MY_Controller {
    ...
    function __construct()
    {
        ...
        $this->data['base_url'] = base_url("lead/sales_spares");
        ...
        $this->data['module_link'] = base_url("lead/sales_spares")."/list_items";
	...
	public function add()
	{
	  ...
	  // $res=mysql_fetch_assoc($result);
	   //pr($res);die;
	  /*$result = $this->sales_spares_mod->get_common();
	...
	{  
		...
		if(check_own_all_permission( ['id'=>$id], $this->table)==false)
        {
            ...
            return redirect(base_url('lead/sales_spares/list_items'));exit;
        }    
       if(isPostBack())
       {
	...
	{  
        ...
        foreach ($result['result'] as $row) { 
            if (!empty($row->display_id)) {
                $row->display_id = '<a href="'.base_url().'lead/sales_spares/view/'.$row->id.'">#'.$row->display_id.'</a>';//'#'.$v[$val['name']];
  ...
  // ------------------------------------------------------------------------
     ...
     */
	  ...
	  return redirect(base_url('lead/sales_spares/view/'.$_POST['lead_id']));
  }
   /**
	...
	public function qualified_form() {
	  ...
	  //pr($this->db->last_query());die;
	  return redirect(base_url('lead/sales_spares/list_items'));
  }
   ...
   /**
	...
	/*public function checkNameExistence() {
		...
		if ($this->input->is_ajax_request()) {
	        $vendor_code = $this->input->get_post('vendor_code', true);
	        $id = $this->input->get_post('id', true);
			$result = $this->sales_spares_mod->check_name_existance($vendor_code, $id);
	        //pr($);die;
	        if ($result == true)
            {
            	echo true;
