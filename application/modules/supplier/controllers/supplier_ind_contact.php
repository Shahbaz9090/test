...
class Supplier_Ind_Contact extends MY_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('supplier_ind_contact_mod');
        ...
        $this->data['base_url'] = base_url("supplier/supplier_ind_contact");
        $this->export_limit = $this->config->item('export_limit');
        $this->delete_limit = $this->config->item('delete_limit');
        $this->table_name   = "supplier_ind_contact";

		$this->data['module'] = 'Supplier/Supplier India';
        $this->data['module_link'] = base_url("supplier/supplier_ind_contact")."/list_items";
	...
	{	
	   if(isPostBack())
       {
            $id = $this->supplier_ind_contact_mod->add();
            if($id){
	   ...
	   $result = $this->supplier_ind_contact_mod->get_vendor();
	   $table_data = $this->supplier_ind_contact_mod->get_table_name();
	   //pr($table_data);die;
       ...
       $this->data['supplier'] = $this->supplier_ind_contact_mod->get_supplier();
       $this->data['department'] = get_all_department_from_masters();
        $this->data['designation'] = get_all_designation_from_masters();
	   $this->data['action'] = "add";
	   $views[] = "supplier_ind_contact/lead_form";
	   $this->data['submodule'] = 'Add ';
	   //$data['industry'] = $this->supplier_ind_contact_mod->get_industry();
	   //pr($this->data);die;
	...
	{        
       $result = $this->supplier_ind_contact_mod->get($id);
       $this->data['result'] = $result;
       $this->data['readonly'] = 'readonly="true"';
       $this->data['action'] = "view";
       $views[] = "supplier_ind_contact/view_lead"; 
		$table_data = $this->supplier_ind_contact_mod->get_table_name();
		$vendor = $this->supplier_ind_contact_mod->get_vendor();	
	...
	{  
        if(isPostBack())
       {
           $r = $this->supplier_ind_contact_mod->update($id);
            if($r){
                redirect($this->data['base_url']."/list_items/");
            }else{
	...
	{  
		...
		$views[] = "supplier_ind_contact/lead_list";
        $this->data['title'] = lang('list_title');
        $this->data['place_holder'] = "Enter filter terms here";        
        $this->data['action'] = "list";

        $this->data['grid']['cols'] = $this->supplier_ind_contact_mod->get_flexigrid_cols();
        ...
        $order_by='id';
        $order='desc';
        $result =  $this->supplier_ind_contact_mod->ajax_list_items($text, $limit, $offset, $order_by, $order,$user);
		//pr($result);die;
		$priority = _priority();
        ...
        $this->data['grid']["page_offset"] = $offset;
        $this->data['grid']["limit"] = $limit;
        //=====pagination link==========//
	...
	{ 
		...
		$perPage = $this->supplier_ind_contact_mod->perPage($user->id);
        ...
        if($perPage) {
            ...
            $pageArr = array(
                ...
                'user_id' => $user->id);
            $this->supplier_ind_contact_mod->insert_perPage($pageArr);
        }
        ...
        $data = $this->supplier_ind_contact_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
       
       
        $data['grid']['total'] = $data['total'];
        $data['grid']['cols'] = $this->supplier_ind_contact_mod->get_flexigrid_cols();
    ...
    public function export($id='')
        {   
            ...
            $result =  $this->supplier_ind_contact_mod->ajax_list_items($text, null, null, null, null,null,$is_export,$items_data);

            $result = $result['result'];
            $table_columns = ["supplier_name"=>"vendor Name","secondary_phone"=>"Mobile No.","email_id"=>"Official Email-Id","name"=>"Name","previous_company"=>"Previous Company","department_name"=>"Department","designation_name"=>"Designation","primary_phone"=>"Personal Mobile No.","personal_email"=>"Personal Email"];  
            $filename = "Supplier India Contact" . date('d-m-Y'). ".xls";
...
/**
    ...
    public function fetch_state_according_country()
    {
       $id          = $this->input->get_post('id',TRUE);
		...
		if(!empty($id)){
			echo $data['value'] = $this->supplier_ind_contact_mod->fetch_state($id);
		}else{
			return false;
		}
    }
    ...
    public function fetch_city_according_country()
    {
       $id          = $this->input->get_post('id',TRUE);
		...
		if(!empty($id)){
			echo $data['value'] = $this->supplier_ind_contact_mod->fetch_city($id);
		}else{
			return false;
		}
    }    
        ...
        $result = $this->supplier_ind_contact_mod->check_vendor_code_existance($vendor_code);
        //pr($result);die;
        if ($result == true)
            echo true;
    }
