	...
	<div class="span12">
		...
		<div class="box">
			...
			<div class="content">
				...
				<?php echo get_flashdata(); //pr($result);?>
				...
				<div class="form-row row-fluid">
					...
					<!--<div class="span6">
						<div class="row-fluid">
							<label class="form-label span4" for="normal"><?=lang('vendor_data')?><em>*</em></label>
							  <div class="row-fluid">
								<div class="span6 ">
								
								<input name="vendor_data" type="text" class="col-xs-10 col-sm-6 pull-right" id="vendor_data" value="<?php if(isset($id)){echo $id;}else if($result->vendor_data){ echo $result->vendor_data;}?>"/>
								</div>
							 </div>
						</div>
					</div>-->
								...
								<div class="span6 select_style_margin-left">  
								  <select name="vendor_type" class="nostyle chosen-select"  id="vendor_type">
										...
										<?php if(isset($vendor) && !empty($vendor)){
											foreach($vendor as $i_key=>$i_val) { //pr($i_val);}die;?>
										...
										<option value="">Select</option>
											<?php if(isset($state) && is_array($state)){
														foreach($state as $i_key => $i_val){ 
															if($i_val->country_name=='China'){
														?>
														<?php $state_id = set_value('state') == false ? @$result->state_id : set_value('state');  ?>
													   <option value="<?=$i_val->id;?>" <?= ($state_id == $i_val->id)?"selected='selected'":"" ?>><?=$i_val->state_name; ?></option>
														<?php } } }?>
										</select>
							  ...
							  <div class="row-fluid">
								...
								<select name="city_comp" id="city_comp" class="nostyle chosen-select" >
											...
											<?php if(isset($city) && is_array($city)){
														...
														?>
														<?php $city_id = set_value('city') == false ? @$result->city_id : set_value('city');  ?>
													   <option value="<?=$i_val->id;?>" <?= ($city_id == $i_val->id)?"selected='selected'":"" ?>><?=$i_val->city_name; ?></option>
											<?php } } ?>
								</select>
						...
						<div class="row-fluid">
							<label class="form-label span4" for="normal"><?=lang('pincode')?><em>*</em></label>
							  <div class="row-fluid">
								...
								<input type="text" id="pincode" value="<?php echo  @$pincode; ?>" name="pincode" class="col-xs-10 col-sm-6 pull-right" />
								<span id="name_existance_error" class="error_form">
				...
				<div class="form-row row-fluid">	
					...
					<div class="span6">
						<div class="row-fluid">
							<label class="form-label span4" for="normal"><?=lang('item_service_description')?><em>*</em></label>
							  <div class="row-fluid">
								<div class="span6" s>
								<?php $item_description = set_value('item_service_description') == false ? @$result->item_description : set_value('item_service_description');  ?>
								<input type="text" id="item_description" value="<?php echo  @$item_description; ?>" name="item_description" class="col-xs-10 col-sm-6 pull-right" />
								<span id="item_description_existance_error" class="error_form">
                                 <small class="error_form"><?php echo form_error('item_service_description'); ?></small>
								 </span>
								</div>
							 </div>
						</div>
				...
				<div class="form-row row-fluid">
					...
					<div class="span6">
						<div class="row-fluid">
							<label class="form-label span4" for="normal"><?=lang('tax_no')?><em>*</em></label>
							  <div class="row-fluid">
								<div class="span6 " s>
								<?php $tax_no = set_value('tax_no') == false ? @$result->tax_no : set_value('tax_no');  ?>
								<!--<input minlength="18" maxlength="18" oninvalid="this.setCustomValidity('Tax No. must be 18 digit')" oninput="this.setCustomValidity('')" type="text" value="<?php echo  @$tax_no; ?>" id="tax_no" name="tax_no" class="col-xs-10 col-sm-6 pull-right"  />-->
								<input type="text" value="<?php echo  @$tax_no; ?>" id="tax_no" name="tax_no" class="col-xs-10 col-sm-6 pull-right"  />
								<span id="tax_no_existance_error" class="error_form">
                                 <small class="error_form"><?php echo form_error('tax_no'); ?></small>
								 </span>
								</div>
							 </div>
						</div>
							...
							<label class="form-label span4" for="normal"><?=lang('bank_name')?><em>*</em></label>
							  <div class="row-fluid">
								...
								<select name="bank_name" class="nostyle chosen-select" data-placeholder="Select..." id="bank_name">
									<option value="">Select</option>
									<?php if(isset($sup_china_bank_name) && is_array($sup_china_bank_name)){
					...
					<div class="row-fluid">
							<label class="form-label span4" for="normal"><?=lang('company_contact_person_mobile')?><em>*</em></label>
							  <div class="row-fluid">
								...
								<input type="text" value="<?php echo  @$result->primary_phone; ?>" id="company_contact_person_phone" name="company_contact_person_phone" class="col-xs-10 col-sm-6 pull-right"  />
								...
								<ul>
									...
									if(isset($result->company_doc) && !empty($result->company_doc))
									{
										foreach ($result->company_doc as $company_doc_key => $company_doc_value) {
											...
											<li>
												<i><img style="height:12px;width:12px;padding-right:3px;position: absolute;margin: auto;bottom: 0;top: 0;" src="<?=base_url('upload/supplier_china/'.$company_doc_value->filename)?>"/></i>
												...
												<a download href="<?=base_url('upload/supplier_china/'.$company_doc_value->filename)?>" class="file-action file-delete"><i class="fa fa-download pull-right"></i></a>
											</li>
...
// get the state from country_name
	function fetch_state(id){
		...
		if(id!=''){
			$.ajax({
						url:"<?php echo base_url();?>supplier/supplier_china/fetch_state_according_country",
                        method:"GET",
                        data:{id:id},
	...
	function fetch_city(id){
		...
		if(id!=''){
			$.ajax({
						url:"<?php echo base_url();?>supplier/supplier_china/fetch_city_according_country",
                        method:"GET",
                        data:{id:id},
						success:function(data)
						{
    ...
    var action="<?=SITE_PATH?>supplier/supplier_china/checkVendorCodeExistence";
    ...
    $.post(action,str,function(data){
        ...
        {
			var error='<label for="vendor_code" generated="true" class="error">This field is required.</label>';
            $("#vendor_code_existance_error").html(error);
            email_existance="yes";
        }
    });
...
function removeFile(obj)
{
	...
	{ 
	    ...
	    $.ajax({  
			method:"GET", 
			dataType:'json', 
			url:"<?php echo base_url();?>supplier/supplier_china/remove_client_doc",
...
function checkuploadDocData(){
  ...
  //status_text_contents = 0;
   ...
   var messageLength = CKEDITOR.instances['note'].getData( ).replace( /<[^>]*>/gi, '' ).length;
	...
	if(item_description==''){
      status_item_description = 0;
      $("#item_description_existance_error").html('Item Description is required');
    }else{
      status_item_description=1;
      $("#item_description_existance_error").html('');
    }
	if(tax_no==''){
      status_tax_no = 0;
      $("#tax_no_existance_error").html('TAX No. is required');
    }else{
      status_tax_no=1;
       $("#tax_no_existance_error").html('');
    }if(bank_name==''){
      status_bank_name = 0;
...
if(action !='edit'){
 ...
 if(status_name == 1 && status_pincode == 1 && status_website_address ==1 && status_state_comp ==1 && status_city_comp ==1 && status_city_code ==1 &&status_item_description==1 && status_tax_no ==1  &&  status_bank_name ==1 &&  status_account_number ==1  && status_doc == 1 &&status_text_contents==1 ){
        status =1;
      }else{
        status = 0;
      }
