...
class Supplier_China_Contact_mod extends CI_Model {

   // public $table     = "companies";
	public $per_page  = "per_page";
    var $table = "supplier_china_contact";
    ...
    function add()
    {
		...
		$contact_add['supplier_id']	=	$_POST['supplier_name'];
		...
		$contact_add['wechat']	=	$_POST['wechat'];
		$contact_add['qq_id']	=	$_POST['qq_id'];
		...
		$id = $this->db->insert('supplier_china_contact',$contact_add);
		add_report($id);  
    ...
    {  
        ...
        $this->db->select('sup.*,supplier_china.name as supplier_name,department.name as department_name,designation.name as designation_name');
        $this->db->join('supplier_china', "supplier_china.id = sup.supplier_id");
        $this->db->join('department', "department.id = sup.department");
        $this->db->join('designation', "designation.id = sup.designation");
        $this->db->from('supplier_china_contact sup');
    ...
    function ajax_list_items($text, $limit, $offset, $order_by='id', $order='desc',$user='',$is_export=false, $items_data = NULL) {
      ...
      //$this->db->select("SQL_CALC_FOUND_ROWS $this->table .*",FALSE);
       ...
       // pr($query->result());exit;
		...
		$this->db->select('SQL_CALC_FOUND_ROWS sup.*,supplier_china.name as supplier_name,department.name as department_name,designation.name as designation_name',false);
        $this->db->from('supplier_china_contact sup');
        $this->db->join('supplier_china', "supplier_china.id = sup.supplier_id");
        $this->db->join('department', "department.id = sup.department");
        $this->db->join('designation', "designation.id = sup.designation");
        ...
        if($this->form_validation->run() == FALSE)
        {
           // set_flashdata("error",validation_errors());
            return FALSE;
				...
				$this->db->where("id",$id);
				$r = $this->db->update('supplier_china_contact',$contact_add);
    ...
    function get_flexigrid_cols()  
    {
           $data = array(
            ...
            array(
                "display"   =>lang('company_contact_email_list'),
                "name"      =>"email_id",
                "order_by" => "yes"
            ),
            array(
            ...
            array(
                "display"   =>lang('company_contact_person_department'),
                "name"      =>"department_name",
                "order_by" => "yes"
            ),
            array(
                "display"   =>lang('company_contact_person_designation'),
	...
	public function check_vendor_code_existance($vendor_code) {
        ...
        $query=$this->db->query("SELECT id FROM supplier_china_contact_contact_contact_contact_contact_contact_contact_contact WHERE vendor_code='$vendor_code'");
    ...
    function get_supplier()
    {
		...
		$this->db->order_by("name","asc");
        $result = $this->db->get("supplier_china");
        if ($result->num_rows > 0) {
            return $result->result();
        } else {
            return false;
