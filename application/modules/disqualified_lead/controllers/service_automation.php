...
class Service_Automation extends MY_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('service_automation_mod');
        ...
        $this->lang->load('service_automation', get_site_language());
               
        $this->data['head']['title'] = "Service Automation";
        $this->data['readonly'] = NULL;
        $this->data['base_url'] = base_url("disqualified_lead/service_automation");
        $this->export_limit = $this->config->item('export_limit');
        $this->delete_limit = $this->config->item('delete_limit');
        $this->table_name   = "leads_service_automation";

		$this->data['module'] = 'Installation Base/Service Automation';
        $this->data['module_link'] = base_url("disqualified_lead/service_automation")."/list_items";
	...
	public function add()
	{
	   ...
	   if(isPostBack())
       {
            $id = $this->service_automation_mod->add();
            if($id){
	   ...
	   $views[] = "service_automation/lead_form";
	   $table_data = $this->service_automation_mod->get_table_name();
		//pr($table_data);die;
	  ...
	  $this->data['submodule'] = 'Add Service Automation';
	  $this->data['industry'] = $this->service_automation_mod->get_industry();
	...
	{        
       $result = $this->service_automation_mod->get($id);
       $this->data['result'] = $result;
       $this->data['readonly'] = 'readonly="true"';
       $this->data['action'] = "view";
       $views[] = "service_automation/view_lead"; 
	   $appoitment = $this->service_automation_mod->appointment_data();
	   ...
	   $table_data = $this->service_automation_mod->get_table_name();
		...
		$note = $this->service_automation_mod->get_notes($id);
	   $this->data['note'] = $note;
	   $appoitment = $this->service_automation_mod->appointment_data();
	   $disqualified_notes = $this->service_automation_mod->disqualified_notes();
	   $notes = $this->service_automation_mod->notes();
	   $product = $this->service_automation_mod->product();
	   $service = $this->service_automation_mod->service();
	   $this->data['service'] = $service;
	   $this->data['product'] = $product;
	   $this->data['notes'] = $notes;
       $this->data['appoitment'] = $appoitment;
       $this->data['disqualified_notes'] = $disqualified_notes;
	   $this->data['submodule'] = 'View Service Automation';
	...
	{   	   
       if(isPostBack())
       {
           $r = $this->service_automation_mod->update($id);
      ...
      $this->data['designation'] = get_all_designation_from_masters();
	   ...
	   $this->data['submodule'] = 'Edit Service Automation';
	   //pr($this->data);die;
	   view_load($views,$this->data);
	...
	{  
		...
		$views[] = "service_automation/lead_list";
        $this->data['title'] = lang('list_title');
        $this->data['place_holder'] = "Enter filter terms here";        
        $this->data['action'] = "list";

        $this->data['grid']['cols'] = $this->service_automation_mod->get_flexigrid_cols();
        ...
        $order_by='id';
        $order='desc';
        $result =  $this->service_automation_mod->ajax_list_items($text, $limit, $offset, $order_by, $order,$user);
		//pr($result['result']);
		$priority = _priority();
        $referalSource = _referalSourceList();
        foreach ($result['result'] as $row) { 
            if (!empty($row->display_id)) {
                $row->display_id = '<a href="'.base_url().'disqualified_lead/service_automation/view/'.$row->id.'">#'.$row->display_id.'</a>';//'#'.$v[$val['name']];
        ...
        $this->data['submodule'] = 'Service Automation';
        view_load($views, $this->data);
	}
	...
	{ 
		...
		$perPage = $this->service_automation_mod->perPage($user->id);
        ...
        if($perPage) {
            ...
            $pageArr = array(
                ...
                'user_id' => $user->id);
            $this->service_automation_mod->insert_perPage($pageArr);
        }
        ...
        $data = $this->service_automation_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
        
       
        $data['grid']['total'] = $data['total'];
        $data['grid']['cols'] = $this->service_automation_mod->get_flexigrid_cols();
    ...
    public function export()
    {
       ...
       $data = $this->service_automation_mod->export();
  ...
  // ------------------------------------------------------------------------
   ...
   public function get_company_all_data()
	...
	public function checkVendorCodeExistence() {
        ...
        $result = $this->service_automation_mod->check_vendor_code_existance($vendor_code);
        //pr($result);die;
        if ($result == true)
            echo true;
    }
     ...
     */
	  ...
	  return redirect(base_url('disqualified_lead/service_automation/view/'.$_POST['lead_id']));
  }
   /**
	...
	public function qualified_form() {
	  ...
	  $data['lead_status'] = '2';
	  $this->db->where('id',$id);
	  $this->db->update('leads_service_automation', $data);
	  //pr($this->db->last_query());die;
	  return redirect(base_url('disqualified_lead/service_automation/list_items'));
  }
...
public function remove_client_doc() {
		...
		if(isset($_GET['leads_service_automation_doc_id']) && !empty($_GET['leads_service_automation_doc_id']) && is_numeric($_GET['leads_service_automation_doc_id']))
		{
			$leads_service_automation_doc_id = $_GET['leads_service_automation_doc_id'];
			$this->db->select('filename');
			$this->db->where('id',$leads_service_automation_doc_id);
			$record = $this->db->get("leads_service_automation_doc")->row();
			if(isset($record->filename) && !empty($record->filename))
			{
				unlink(FCPATH.'upload/service_automation/'.$record->filename);
				$this->db->where('id',$leads_service_automation_doc_id);
				$this->db->delete('leads_service_automation_doc');
	...
	public function checkNameExistence() {
        ...
        $id = $this->input->get_post('id', true);
		$result = $this->service_automation_mod->check_name_existance($company_text,$id);
        //pr($result);die;
        if ($result == true)
            echo true;
    }
