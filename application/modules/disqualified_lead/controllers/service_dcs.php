...
class Service_Dcs extends MY_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model('service_dcs_mod');
        ...
        $this->lang->load('service_dcs', get_site_language());
        $this->data['head']['title'] = "Service DCS";
        $this->data['readonly'] = NULL;
        $this->data['base_url'] = base_url("disqualified_lead/service_dcs");
        $this->export_limit = $this->config->item('export_limit');
        $this->delete_limit = $this->config->item('delete_limit');
        $this->table_name   = "leads_service_dcs";

		$this->data['module'] = 'Installation Base/Service DCS';
        $this->data['module_link'] = base_url("disqualified_lead/service_dcs")."/list_items";
	...
	public function add()
	{
	   ...
	   $views[] = "service_dcs/lead_form";
	   $table_data = $this->service_dcs_mod->get_table_name();
		//pr($table_data);die;
	  ...
	  $this->data['submodule'] = 'Add Service DCS';
	  $this->data['industry'] = $this->service_dcs_mod->get_industry();
	...
	{        
       $result = $this->service_dcs_mod->get($id);
       $this->data['result'] = $result;
       $this->data['readonly'] = 'readonly="true"';
       $this->data['action'] = "view";
       $views[] = "service_dcs/view_lead"; 
	   $appoitment = $this->service_dcs_mod->appointment_data();
	   ...
	   $table_data = $this->service_dcs_mod->get_table_name();
		...
		$note = $this->service_dcs_mod->get_notes($id);
	   $this->data['note'] = $note;
	   $appoitment = $this->service_dcs_mod->appointment_data();
	   $disqualified_notes = $this->service_dcs_mod->disqualified_notes();
       $this->data['disqualified_notes'] = $disqualified_notes;
       $this->data['appoitment'] = $appoitment;
	   $notes = $this->service_dcs_mod->notes();
	   $product = $this->service_dcs_mod->product();
	   $service = $this->service_dcs_mod->service();
	   $this->data['service'] = $service;
	   $this->data['product'] = $product;
	   $this->data['notes'] = $notes;
	   $this->data['submodule'] = 'View Service DCS';
	...
	{   	   
       if(isPostBack())
       {
           $r = $this->service_dcs_mod->update($id);
      ...
      $this->data['designation'] = get_all_designation_from_masters();
	   ...
	   $this->data['submodule'] = 'Edit Service DCS';
	  // pr($this->data);die;
	   view_load($views,$this->data);
	...
	{  
		...
		$views[] = "service_dcs/lead_list";
        $this->data['title'] = lang('list_title');
        $this->data['place_holder'] = "Enter filter terms here";        
        $this->data['action'] = "list";

        $this->data['grid']['cols'] = $this->service_dcs_mod->get_flexigrid_cols();
        ...
        $order_by='id';
        $order='desc';
        $result =  $this->service_dcs_mod->ajax_list_items($text, $limit, $offset, $order_by, $order,$user);
        ...
        foreach ($result['result'] as $row) { 
            if (!empty($row->display_id)) {
                $row->display_id = '<a href="'.base_url().'qualified_lead/service_dcs/view/'.$row->id.'">#'.$row->display_id.'</a>';//'#'.$v[$val['name']];
        ...
        $this->data['submodule'] = 'Service DCS';
	...
	{ 
		...
		$perPage = $this->service_dcs_mod->perPage($user->id);
        ...
        if($perPage) {
            ...
            $pageArr = array(
                ...
                'user_id' => $user->id);
            $this->service_dcs_mod->insert_perPage($pageArr);
        }
        ...
        $data = $this->service_dcs_mod->ajax_list_items($text, $limit, $offset, $order_by, $order);
        
       
        $data['grid']['total'] = $data['total'];
        $data['grid']['cols'] = $this->service_dcs_mod->get_flexigrid_cols();
    ...
    public function export()
    {
       ...
       $data = $this->service_dcs_mod->export();

       export_report($items_data);
       array_to_csv($data,"Service_dcs.csv");
    }
  ...
  // ------------------------------------------------------------------------
	...
	public function checkVendorCodeExistence() {
        ...
        $result = $this->service_dcs_mod->check_vendor_code_existance($vendor_code);
        //pr($result);die;
        if ($result == true)
            echo true;
    }
     ...
     */
	  ...
	  return redirect(base_url('disqualified_lead/service_dcs/view/'.$_POST['lead_id']));
  }
   /**
	...
	public function qualified_form() {
	  ...
	  $data['lead_status'] = '2';
	  $this->db->where('id',$id);
	  $this->db->update('leads_service_dcs', $data);
	  //pr($this->db->last_query());die;
	  return redirect(base_url('disqualified_lead/service_dcs/list_items'));
  }
  /**
     * Add Product
     *
	...
	public function remove_client_doc() {
		...
		if(isset($_GET['leads_service_dcs_doc_id']) && !empty($_GET['leads_service_dcs_doc_id']) && is_numeric($_GET['leads_service_dcs_doc_id']))
		{
			$leads_service_dcs_doc_id = $_GET['leads_service_dcs_doc_id'];
			$this->db->select('filename');
			$this->db->where('id',$leads_service_dcs_doc_id);
			$record = $this->db->get("leads_service_dcs_doc")->row();
			if(isset($record->filename) && !empty($record->filename))
			{
				unlink(FCPATH.'upload/service_dcs/'.$record->filename);
				$this->db->where('id',$leads_service_dcs_doc_id);
				$this->db->delete('leads_service_dcs_doc');
	...
	public function checkNameExistence() {
        ...
        $id = $this->input->get_post('id', true);
		$result = $this->service_dcs_mod->check_name_existance($company_text,$id);
        //pr($result);die;
        if ($result == true)
            echo true;
    }
