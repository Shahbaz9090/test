...
/**
 * CodeIgniter disqualified_lead Controller
 *
 * @package		CodeIgniter
...
class Sales_Spares extends MY_Controller {
    ...
    function __construct()
    {
        ...
        $this->data['base_url'] = base_url("disqualified_lead/sales_spares");
        ...
        $this->data['module_link'] = base_url("disqualified_lead/sales_spares")."/list_items";
	...
	{        
	   ...
	   $disqualified_notes = $this->sales_spares_mod->disqualified_notes();
	...
	{  
		...
		if(check_own_all_permission( ['id'=>$id], $this->table)==false)
        {
            ...
            return redirect(base_url('disqualified_lead/sales_spares/list_items'));exit;
        } 	   
       if(isPostBack())
       {
	...
	{  
        ...
        foreach ($result['result'] as $row) { 
            if (!empty($row->display_id)) {
                $row->display_id = '<a href="'.base_url().'disqualified_lead/sales_spares/view/'.$row->id.'">#'.$row->display_id.'</a>';//'#'.$v[$val['name']];
  ...
  // ------------------------------------------------------------------------
     ...
     */
	  ...
	  return redirect(base_url('disqualified_lead/sales_spares/view/'.$_POST['lead_id']));
  }
   /**
	...
	public function qualified_form() {
	  ...
	  return redirect(base_url('disqualified_lead/sales_spares/list_items'));
  }
  /**
     * Add Product
     *
