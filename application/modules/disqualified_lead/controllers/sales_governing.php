...
class Sales_Governing extends MY_Controller {
    ...
    function __construct()
    {
        ...
        $this->data['base_url'] = base_url("disqualified_lead/sales_governing");
        ...
        $this->data['module_link'] = base_url("disqualified_lead/sales_governing")."/list_items";
	...
	{   
		...
		if(check_own_all_permission( ['id'=>$id], $this->table)==false)
        {
            ...
            return redirect(base_url('disqualified_lead/sales_governing/list_items'));exit;
        }	   
       if(isPostBack())
       {
	...
	{  
        ...
        foreach ($result['result'] as $row) { 
            if (!empty($row->display_id)) {
                $row->display_id = '<a href="'.base_url().'disqualified_lead/sales_governing/view/'.$row->id.'">#'.$row->display_id.'</a>';//'#'.$v[$val['name']];
			...
			if (!empty($row->assigned_telemarketer)) { // for assign telemarketer value
				...
				/*if ($row->lead_status==1 || $row->lead_status) {
  ...
  // ------------------------------------------------------------------------
     ...
     */
	  ...
	  return redirect(base_url('disqualified_lead/sales_governing/view/'.$_POST['lead_id']));
  }
     /**
	...
	public function qualified_form() {
	  ...
	  //pr($this->db->last_query());die;
	  return redirect(base_url('disqualified_lead/sales_governing/list_items'));
  }
