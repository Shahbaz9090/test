...
class Imap
{
	...
	public function connect(array $config = [])
	{
		...
		if (isset($config['encrypto']) && !empty($config['encrypto']))
		{
			$enc .= '/' . $config['encrypto'];
		}
        ...
        {
            $this->mailbox = $this->mailbox.$config['folder'];
        }
        // echo $this->mailbox;die;
	 ...
	 * @return array
	 */
	public function get_attachments( $uid,  $indexes = [])
	{
		$attachments = [];
	...
	protected function _get_attachments( $uid, $structure,  $part_number = '',	 $index = null)
	{
		...
		else
		{
			...
			if (isset($structure->dparameters[0]))
			{
				...
				$filename1    = mb_decode_mimeheader($bodystruct->dparameters[0]->value);
	...
	public function get_message( $uid)
	{
		...
		$email = [
            ...
            'body'        => mb_convert_encoding($this->get_body($uid)['html'], 'UTF-8', 'GB2312')
		];
	 ...
	 * @return array
	 */
	public function paginate( $uids,  $page = 1,  $per_page = 10)
	{
	...
	protected function to_address($headerinfos)
	{
		...
		if (! empty($headerinfos->personal))
		{
            ...
            $name         = mb_decode_mimeheader($headerinfos->personal);
			// $name         = $name[0]->text;
